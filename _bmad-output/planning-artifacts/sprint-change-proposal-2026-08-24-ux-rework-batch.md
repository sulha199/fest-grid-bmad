...
- **Track A (PM/Architect first):** Items #14 (system key + `changedByUserId` schema gap) and #4/#5 (IA restructuring + nested-tabs question). Route to **Product Manager** (`bmad-prd`, to formally amend §3.7/§4.5/§4.14) and **Architect** (`bmad-architecture`, for the system-key addendum), then a scoped **`bmad-ux`** pass for Item #11's multi-day spanning (the Posts-tab nesting question was separately resolved directly, see the first addendum below), before any of these become stories via `bmad-create-story`. **Track A fully done 2026-08-24** — PM half: §3.7/§4.5/§4.14 amended (`0f48abe`; §6 needed no edit, just careful §3.7 wording so the system-key exception doesn't read as Phase 2 launching early), plus `epics.md` FR89-92 backfilled (`68c99c4`) to keep the FR Coverage Map in sync per this project's own established pairing convention. Architect half: `AD-10` added to `festgrid-architecture-spine.md` (`SYSTEM_GEMINI_API_KEY` as a Secrets Manager credential matching `GEOAPIFY_API_KEY`'s classification; a new sibling `callGeminiForLocationInference()` function rather than modifying `callGemini` itself, structurally fencing the system key off from Story 3.6's general extraction path; no additional rate-limiting, since persisting the inferred result to `defaultLocation` already bounds the call to at most once per unset account). Story `3.4m` is unblocked — still a `sprint-status.yaml` backlog placeholder, ready for `bmad-create-story` whenever picked up. The `bmad-ux` pass for Item #11 (mobile multi-day-span calendar rendering) remains the one open Track A/B item with no design yet.
...
### Addendum to the addendum (2026-08-24, later same day): `GridContainer` primitive

Story 1.3d's AC14 and Story 5.1's AC12 each independently widened their own literal `grid-cols-*` string to the same shape. Rather than ship two copies, a second new primitive — **Story 0.31, `GridContainer`** (`packages/ui/src/core/`) — takes `baseCols`/`colsStep` props (both default `1`) and computes the column count at each Tailwind breakpoint via an internal literal-string lookup table (Tailwind's compiler can't see a dynamically-interpolated class name, only a pre-written literal one — the props just select which). `baseCols=1, colsStep=1` is the standard grid; `baseCols=2, colsStep=1` (one column ahead at every step) is the masonry mode's already-locked progression, whenever that gets built. Both `1.3d`'s AC14 and `5.1`'s AC12 are revised again to compose this primitive instead of their own literal strings — no code existed against either prior version, so this is a documentation correction, not rework. `project-context.md`'s "Grid/Calendar Page Containers" rule and `DESIGN.md`'s `grid.base`/`grid.masonry` tokens updated to match.
