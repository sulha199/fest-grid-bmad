...
**Root-caused further, 2026-09-12: the `@cline/sdk` stall is confirmed SDK-path-specific, not model/provider/prompt-specific — but the CLI alternative was investigated and rejected.** Three real `bmad-dev-story` runs against DeepInfra (`openai-compatible`) reproduced the exact same silent-stall symptom as the earlier `gemini-3.5-flash-lite` anomaly: `session started`, then zero further SDK events until the watchdog killed it — no error, no partial output. Live-isolated: the *exact* reconstructed prompt (same provider/model/`--thinking medium`), run instead through the plain `cline` CLI, produced its first token in ~8 seconds and proceeded normally — same key, same moment, same everything except SDK-vs-CLI. Two different models (gemini and DeepSeek) now reproduce this exclusively through `ClineCore`/`cline.start()`, never through the CLI — strong evidence the bug is in `@cline/sdk`'s embedding layer itself. Also confirmed: a stalled `core`-source session leaves **zero** trace in `cline history` or `~/.cline/data/sessions/<id>/` — there is no way to introspect a stuck session after the fact via any of Cline's own tooling.

This pointed at switching `run-ritual-cline.ts` to shell out to the CLI instead of driving `ClineCore` in-process (reusing the MCP-custom-tool mechanism already proven live for the `--id`-resume investigation above, minus the resume goal this time). Built and live-tested a real `ask_user_question` stdio MCP server (env-var-driven mailbox dir/label, registered via `cline mcp add`) — but the test surfaced a different, more serious problem: given an explicit, correctly-worded instruction naming the tool and its MCP server, the model instead called Cline's own **built-in** `ask_question` tool (`DefaultToolNames.ASK` in `@cline/core`, shared by both the CLI and the SDK — not a CLI-only addition) and it **self-resolved instantly with no mailbox interaction at all** — no pending-request file was ever written. In headless/`--auto-approve` mode this built-in tool does not block for a real human; it fabricates an answer and moves on. That silently defeats the entire point of the mailbox relay (escalating genuine design ambiguity to a person) — a correctness hazard, not just a reliability one, and worse than the stall it would have fixed. No CLI flag was found to disable this built-in tool. **Rejected**: reverted the MCP server, its registration, and the dependency addition; `run-ritual-cline.ts` stays on `ClineCore`/`@cline/sdk`, mitigated by the retry+idle-watchdog above rather than a CLI-based rewrite. Open question, not chased further: whether the same built-in `ask_question` tool is also reachable on the SDK path for some models — the 2026-09-03 verification notes above show a correctly-written mailbox request for the models tested then, so this isn't a known regression there, but it hasn't been re-tested against every model/config combination since.
