...
- [x] Task 2 — Reconcile Story 0.43 against the new rule (AC: #6) — (pre-satisfied by `dc39a681`; verified only this session, not re-performed)
...
#### Code review — 2026-09-22 (layers: Blind Hunter + Edge Case Hunter + Acceptance Auditor; 0 decision-needed, 8 patch, 0 defer, 4 dismissed)

- [x] [Review][Patch] Contradictory unit-test rule: `packages/domain` bullet still says unit tests may be written "only" there, directly above a tier mandating unit tests in tooling packages — amend the domain bullet to scope it to product/domain logic — high — [_bmad-output/project-context.md:146] — **Resolved:** scoped to "Among application/domain-logic code" with a forward pointer to the tooling tier.
- [x] [Review][Patch] Classification test has no tie-break for straddling packages (tooling package with any production surface, or pure logic depending on AST/browser tooling) — add a tie-break rule (production-bundled parts follow tier 1/2; remainder this tier) and clarify the "no browser/AST-tool dependency" Edge of "pure logic" — medium — [_bmad-output/project-context.md:149-153] — **Resolved:** added a "Straddling case" rule to the classification test.
- [x] [Review][Patch] "Coverage judged qualitatively" has no verifiable rubric — add a minimal criterion (e.g. each pure-logic branch class maps to a named `*.test.ts` case recorded in the package's test decision notes) — medium — [_bmad-output/project-context.md:150] — **Resolved:** added the named-branch-to-named-test-case minimal criterion verbatim.
- [x] [Review][Patch] Manifest-entry DoD "running successfully (not just existing)" names no re-verification mechanism (who/what re-runs manifests; via which gate) — state the mechanism so the DoD is enforceable beyond first-pass — medium — [_bmad-output/project-context.md:152] — **Resolved:** stated re-verification rides every consuming story's own `run-check.ts --kind test` gate, not a separate check.
- [x] [Review][Patch] `tsx --test` is mandated with no escape clause for pure logic that legitimately needs Vitest-only features (mocks/snapshots) — add an escape clause requiring documented justification in the story's Dev Notes — low — [_bmad-output/project-context.md:149] — **Resolved:** added the escape clause with the Dev Notes justification requirement.
- [x] [Review][Patch] Task 2 checkbox `[x]` marks work (0.43 amendment) actually performed in prior commit `dc39a681`, this session only verified it — annotate the checkbox "(pre-satisfied by dc39a681; verified only)" to keep the audit trail honest — low — [_bmad-output/implementation-artifacts/0-44-define-testing-standard-for-meta-testing-tooling-packages.md:42] — **Resolved.**
- [x] [Review][Patch] Change Log records the `pnpm install --frozen-lockfile` environment repair but the Dev Agent Record doesn't note it changed no tracked files (only the pre-existing `apps/web/tsconfig.tsbuildinfo` toil) — add a one-line clarification — low — [_bmad-output/implementation-artifacts/0-44-define-testing-standard-for-meta-testing-tooling-packages.md:147] — **Resolved.**
- [x] [Review][Patch] sprint-status.yaml lacks a note that 0.44 is the hard prerequisite of 0.43 (Pre-Coding Approval Gate) — add a comment so future readers see the ordering — low — [_bmad-output/implementation-artifacts/sprint-status.yaml:328] — **Resolved** (comment added on Story 0.43's own line, verified clean via `sprint-status-comment-check.py`).

Dismissed as noise (4): tier text already uses "e.g." for Playwright/ts-morph specifics (already generalized); `baseline_commit` frontmatter is the review workflow's own read convention; tracked `apps/web/tsbuildinfo` churn is pre-existing, unrelated to this commit; "1–2 example entries" borrows 0.43's convention and is adequately example-scoped in the tier text.
