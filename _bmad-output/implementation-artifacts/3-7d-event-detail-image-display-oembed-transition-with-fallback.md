...
- [ ] 10a. **(AC10)** Add a route-scoped Content-Security-Policy for the event-detail route permitting Instagram's embed resources (`script-src`, `frame-src`/`child-src`, `img-src`/`connect-src` per AC10's exact origin list) — implement via `next.config.ts`'s `headers()` (preferred if the route pattern is static) or a dedicated, clearly-commented branch in `apps/web/middleware.ts` that does not share code paths with Story 6.7a's `/widget/[id]` `frame-ancestors` handling. Document the final directive set and origins in this story's Dev Agent Record.
- [ ] 10b. **(Testing, AC10)** Add an integration/E2E assertion (Playwright, alongside the MSW-backed test in Task 14) that loads the event-detail page with an `AVAILABLE` embed under the app's real CSP headers and confirms the Instagram iframe/script render with no CSP violation reported (e.g. no `securitypolicyviolation` event / no console CSP error).
...
- [ ] E2E/integration test confirming the event-detail route's CSP allows the Instagram embed to render with no CSP violation (AC10, Task 10b).
...
- [ ] Event-detail route's CSP explicitly permits Instagram's embed script/iframe/media origins, verified with no CSP violation at runtime (AC10).
