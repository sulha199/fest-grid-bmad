...
async function runWithTestProfile(fn: (profileId: string) => Promise<void>) {
  const profileId = randomUUID();
  await db.insert(socialMediaAccountProfiles).values({
    id: profileId,
    accountId: 'acct-' + Date.now() + '-' + randomUUID(),
    platform: 'instagram',
    username: 'test_user_' + Date.now() + '_' + randomUUID().substring(0, 8),
  ...
  try {
    ...
    await db.delete(unprocessedScraperPayloads).where(
      or(
        sql`context->>'accountId' = ${profileId}`,
        sql`(("context"#>>'{}')::jsonb)->>'accountId' = ${profileId}`
      )
    );
    await db.delete(scraperActorRuns).where(eq(scraperActorRuns.profileId, profileId));
...
test('scraper actor run linking (Story 3-4l)', async (t) => {
  ...
  await t.test('persistScrapedPost accepts and writes scraperActorRunId', async () => {
    await runWithTestProfile(async (testProfileId) => {
      // Create an audit run
  ...
  await t.test('Multiple posts from same run are linked correctly', async () => {
    await runWithTestProfile(async (testProfileId) => {
      ...
      const postUrl1 = `https://instagram.com/p/test1-${Date.now()}-${randomUUID()}`;
      const postUrl2 = `https://instagram.com/p/test2-${Date.now()}-${randomUUID()}`;
  ...
  await t.test('replayActorRun Apify branch passes videoUrl and originalPostUrl', async () => {
    await runWithTestProfile(async (testProfileId) => {
      const auditRunId = await recordActorRunStart({
        ...
        triggerMode: 'SYNC',
        profileId: testProfileId,
        runId: 'apify-run-replay-' + Date.now() + '-' + randomUUID(),
        rawInput: { test: true },
      });

      assert.ok(auditRunId);

      const replayPostUrl = `https://instagram.com/p/apify-replay-url-${Date.now()}-${randomUUID()}/`;
      ...
      const [persisted] = await db
        .select()
        .from(posts)
        .where(eq(posts.postUrl, replayPostUrl))
        .limit(1);

      assert.ok(persisted);
      assert.strictEqual(persisted.videoUrl, 'https://example.com/apify-video.mp4');
      assert.strictEqual(persisted.originalPostUrl, replayPostUrl);
    });
  });
  ...
  await t.test('replayActorRun Bright Data branch passes videoUrl and originalPostUrl', async () => {
    await runWithTestProfile(async (testProfileId) => {
      const auditRunId = await recordActorRunStart({
        ...
        triggerMode: 'SYNC',
        profileId: testProfileId,
        runId: 'bd-run-replay-' + Date.now() + '-' + randomUUID(),
        rawInput: { test: true },
      });

      assert.ok(auditRunId);

      const replayPostUrl = `https://instagram.com/p/bd-replay-url-${Date.now()}-${randomUUID()}/`;
