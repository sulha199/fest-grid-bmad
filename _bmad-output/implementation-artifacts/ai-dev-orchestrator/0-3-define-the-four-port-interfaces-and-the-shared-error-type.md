...
- [ ] Task 1: Create `ai-dev-orchestrator/src/core/ports/orchestrator-error.ts` defining `OrchestratorError` extending the native `Error`. (AC: 5)
- [ ] Task 2: Create `ai-dev-orchestrator/src/core/ports/llm-port.ts` defining `LLMPort` interface with `complete` method. (AC: 3)
- [ ] Task 3: Create `ai-dev-orchestrator/src/core/ports/exec-port.ts` defining `ExecPort` interface with `run`, `readFile`, `writeIfUnchanged`, `getWrittenPaths`, and `resetWrittenPaths`. (AC: 3, 4)
- [ ] Task 4: Create `ai-dev-orchestrator/src/core/ports/notify-port.ts` defining `NotifyPort` interface with `send` method. (AC: 3)
- [ ] Task 5: Create `ai-dev-orchestrator/src/core/ports/hitl-port.ts` defining `HITLPort` interface with `prompt` method. (AC: 3)
- [ ] Task 6: Create a type-level unit test file `ai-dev-orchestrator/src/core/ports/ports.test.ts` utilizing Vitest to assert interface contracts, return shapes, and compilation correctness of the ports and `OrchestratorError`. (AC: 2, 3, 4, 5)
- [ ] Task 7: Run TypeScript checks and Vitest tests to ensure that everything is strictly typed, compliant, and compile-safe. (AC: 2, 7)
