...
- **Scoped Exception — Confirm-then-Refetch Toggle (added 2026-09-18, `bmad-correct-course`, FIND-022):** The event/post-detail coauthor/publisher subscribe-unsubscribe toggle (Story 0.i6g) is **confirm-then-refetch**, not optimistic — confirm the mutation, refetch/invalidate authoritative state on success, restore prior state on failure. This is a deliberate, scoped exception to this project's default optimistic-mutation convention (e.g. `toggleFavorite`'s ±1 pattern, BUG-008/Story 2.i1a) and does **not** generalize to any other toggle. Do not cite this exception as precedent for converting another mutation to confirm-then-refetch, and do not cite the optimistic default as grounds to revert this toggle — each is scoped to its own mutation.
...
- **Unit Test Requirement:** All logic exported from `packages/domain` **must** have 100% unit test coverage. Among application/domain-logic code, this is the *only* place where unit tests should be written (see the Meta-Testing / Tooling Packages tier below for the one other place unit tests are written in this codebase).
...
- **Meta-Testing / Tooling Packages:** A package whose job is to help verify *other* code (e.g. `packages/visual-audit`) rather than implement application/domain logic or a user-facing `apps/*` surface itself is neither `packages/domain` nor `apps/*` — it is this third tier:
    - **Unit tests:** the package's own pure logic (comparison/tolerance/clustering/parsing functions — anything with no browser/AST-tool dependency) **must** have unit tests, written with the `tsx --test *.test.ts` pattern already established by `packages/graphql-select` — not Vitest/testing-trophy, and not `packages/domain`'s 100%-coverage rule. Escape clause: if a piece of pure logic genuinely needs a Vitest-only feature (mocks, snapshots), Vitest may be used instead, but the story's Dev Notes must record the specific feature that justified the exception — this is not a general opt-out of the `tsx --test` default.
    - **Coverage bar:** the 100%-coverage rule does **not** apply here. Coverage is judged qualitatively (are the pure-logic branches that matter covered), matching `packages/graphql-select`'s own unenforced-percentage precedent — the minimal bar is that each pure-logic branch class the package's own Dev Notes/test-decision record identifies as meaningful maps to a named `*.test.ts` case; an unrecorded branch is the actual gap this bar exists to catch, not an unbounded judgment call.
    - **Browser-automation / AST-analysis surfaces:** code that itself drives browser automation or static-analysis tooling (e.g. Playwright's `fixtures.mount()` render harness and `toHaveScreenshot()` pixel-diff wiring, or `ts-morph` AST branch-enumeration) is **not** required to carry its own unit tests in the traditional sense — mocking away the tool being verified (e.g. Playwright's `getComputedStyle()` or `ts-morph`'s AST walker) would validate the mock, not the tool. Instead, the package's own example/proof manifest entries running end-to-end against a real fixture serve as this surface's integration proof, and running them successfully (not just existing) is a Definition-of-Done requirement — re-verified the same way as any other test, i.e. every consuming story's own `run-check.ts --kind test` pass re-runs the manifests as part of that gate, not a separate one-off check.
    - **Classification test:** a package belongs in this tier if it ships zero production UI/API surface, is never bundled into an `apps/*` production build, and exists to verify other code's behavior rather than implement product behavior. Straddling case: if a package is mostly this tier but also ships a production-bundled part (an actual UI/API surface consumed by an `apps/*` build), that shipped part follows the `packages/domain`/`apps/*` tier that governs it instead — this tier applies only to the verification-tooling remainder, never to a package's production-facing slice.
