...
- **Meta-Testing / Tooling Packages:** A package whose job is to help verify *other* code (e.g. `packages/visual-audit`) rather than implement application/domain logic or a user-facing `apps/*` surface itself is neither `packages/domain` nor `apps/*` — it is this third tier:
    - **Unit tests:** the package's own pure logic (comparison/tolerance/clustering/parsing functions — anything with no browser/AST-tool dependency) **must** have unit tests, written with the `tsx --test *.test.ts` pattern already established by `packages/graphql-select` — not Vitest/testing-trophy, and not `packages/domain`'s 100%-coverage rule.
    - **Coverage bar:** the 100%-coverage rule does **not** apply here. Coverage is judged qualitatively (are the pure-logic branches that matter covered), matching `packages/graphql-select`'s own unenforced-percentage precedent.
    - **Browser-automation / AST-analysis surfaces:** code that itself drives browser automation or static-analysis tooling (e.g. Playwright's `fixtures.mount()` render harness and `toHaveScreenshot()` pixel-diff wiring, or `ts-morph` AST branch-enumeration) is **not** required to carry its own unit tests in the traditional sense — mocking away the tool being verified (e.g. Playwright's `getComputedStyle()` or `ts-morph`'s AST walker) would validate the mock, not the tool. Instead, the package's own example/proof manifest entries running end-to-end against a real fixture serve as this surface's integration proof, and running them successfully (not just existing) is a Definition-of-Done requirement.
    - **Classification test:** a package belongs in this tier if it ships zero production UI/API surface, is never bundled into an `apps/*` production build, and exists to verify other code's behavior rather than implement product behavior.
