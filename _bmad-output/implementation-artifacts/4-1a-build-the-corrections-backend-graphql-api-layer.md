...
7. **And** corrections that fail any check (AC3–AC5) are **not** applied to `events`/`schedules`; a `corrections` row is still inserted with `status: 'rejected'` and `resolvedAt: now()`, and the mutation's GraphQL response includes a `validationErrors: [String!]` field listing every AJV/consistency/ownership error from AC3–AC5. This field is computed by the resolver at request time from the AJV/domain-check results — it is **not** a persisted `corrections` table column (AC1's column list is exhaustive).
...
- [ ] **Task 5 (AC2, AC5, AC6, AC7) — Resolver:** In `apps/backend/src/schema/resolvers.ts`:
  ...
  - [ ] Add `submitCorrection: async (_: any, { eventId, proposedData, source }: any, context: any) => { ... }` to the `Mutation` resolver map:
    ...
    3. Run `validateProposedEventCorrection(proposedData)`; collect any AJV errors (`validateProposedEventCorrection.errors`) into a `validationErrors: string[]` array (AC3).
    4. If AJV passed, run `validateCorrectionConsistency(proposedData)` and append its messages (AC4).
    5. If AJV passed, check every `proposedData.schedules[].id` (where present) belongs to a `schedules` row with `eventId` matching the argument (via `inArray` + `and(eq(...))`); append a message for any that don't (AC5).
...
- [ ] **Task 6 (AC1–AC8) — Tests:** Create `apps/backend/src/schema/corrections.test.ts` (real local DB, `graphql-yoga` `createSchema`/`createYoga`, mirroring `system-errors.test.ts`'s harness and `favorites-and-calendar.test.ts`'s real-DB seed/cleanup pattern): unauthenticated call rejected `UNAUTHENTICATED`; unknown `eventId` rejected `NOT_FOUND`, no `corrections` row inserted; a structurally-invalid `proposedData` (e.g. empty `eventName`) returns `status: 'rejected'` with matching `validationErrors`, `events` unchanged; a structurally-valid but inconsistent `proposedData` (end date before start date; two main schedules; mismatched schedule location) each independently return `status: 'rejected'` with the specific expected message, `events`/`schedules` unchanged; a schedule `id` from a different event is rejected with an ownership error; a fully valid `proposedData` returns `status: 'applied'`, `validationErrors: []`, and `events`/`schedules` rows are verified actually updated (including a new schedule inserted for an entry with no `id`, and an existing schedule preserved when omitted from the array).
...
- **Compatibility finding: one new DB table + two new enums (additive, no existing-table changes); two new `packages/domain` interfaces; two new GraphQL input types + one new GraphQL output type; no `packages/shared-types` change.**
...
- [ ] `apps/backend/src/schema/corrections.test.ts` (new, real local DB, `graphql-yoga` harness mirroring `system-errors.test.ts`): unauthenticated call rejected `UNAUTHENTICATED`; unknown `eventId` rejected `NOT_FOUND` with no `corrections` row inserted; AJV-structural failure (empty `eventName`) returns `status: 'rejected'` with matching `validationErrors`; each domain-consistency failure (date ordering, main-schedule count, location mismatch) independently returns `status: 'rejected'` with its specific message; a schedule `id` belonging to a different event is rejected with an ownership error; a fully valid submission returns `status: 'applied'`, `validationErrors: []`, and actually updates `events`/inserts-or-updates `schedules` rows (including preserving a `schedules` row omitted from the array).
