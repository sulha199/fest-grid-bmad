...
6. **And** (added 2026-08-19): the `callApifyActor` function and all call sites enforce **strict, type-safe actor input validation** based on `actorId`. This is achieved by:
   - Creating an `ActorRegistry` type that maps each actor ID to its corresponding input schema type  
   - Making `callApifyActor` a generic function: `callApifyActor<T extends ActorId>(actorId: T, input: ActorInputFor<T>): Promise<any[]>`, where `ActorInputFor<T>` resolves to the correct input type for the given actor ID
   - Defining actor-specific input types (`GetPostByUrlActorInput`, `LookupAccountProfileActorInput`, `GetNewestPostsActorInput`) that capture the shape each actor expects  
   - All three adapter methods (`getPostByUrl`, `lookupAccountProfile`, `getNewestPosts`) pass strongly-typed input objects to `callApifyActor` — TypeScript enforces at compile time that the input shape matches the actor's expected schema
   - Result: a future actor swap (AC4) is a safe one-line constant change with full compile-time type checking; any input shape mismatch is caught immediately at the call site, not at runtime
7. ~~**Added 2026-08-15, per confirmed Task 1b finding #7...**~~ **SPLIT OUT to Story 3.4e, 2026-08-15 (Sprint Change Proposal `sprint-change-proposal-2026-08-15-not-found-detection-bug.md`).** This AC — the not-found-detection fix for `getPostByUrl`/`lookupAccountProfile` — is no longer this story's scope. Rationale: it's a confirmed, already-live data-integrity bug (Story 6.1a ships `review` status with the fabrication defect this AC would fix), not a cost/reliability optimization like the rest of this story — bundling it here would make it wait on this story's slower, non-urgent actor-selection work. See Story 3.4e (`epics.md`) for the carried-forward AC content, full evidence, and fix spec. This story's AC1-5/AC6 are unaffected.
