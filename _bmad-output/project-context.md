...
- **Grid/Calendar Page Containers:** (added 2026-08-24, `bmad-correct-course`) Any page whose primary content is a card grid or calendar (Discovery, Favorites, Feed, Archive, the public account page, My Calendar, Manual Post Selection — not limited to this list) **must** use the shared `PageContainer` primitive (`packages/ui/src/core/page-container.tsx`, Story 0.30) instead of a page-local `max-w-* mx-auto` wrapper. Found as a copy-pasted `"p-4 sm:p-8 space-y-8 max-w-7xl mx-auto"` className across 7 files before this rule existed — the duplication itself, not just the width cap, was the defect.
  ...
  - **Card-grid column count must scale with width, not cap at `lg:`.** `EventListView`'s (Story 1.3d) two view modes each have their own progression, one column apart at every step (the Pinterest/masonry mode's "2 cols mobile" vs. the standard mode's "1 col mobile"):
    - **Standard card grid:** `grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5`.
    - **Masonry/Pinterest grid** (Story 1.3b/1.3d, item #10 of `ux-rework-2026-08-24.md`): `grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6`.
    - Other card grids in the app (e.g. Manual Post Selection's `PostCard` grid) should adopt the **standard** progression above rather than inventing a third scheme, unless a story explicitly documents why it needs to diverge.
