...
- **`Query.events` (`getEvents`) is the highest-traffic endpoint in the system — must be kept highly optimized** (added 2026-09-15, `bmad-agent-architect` review). It is the shared backing query for Discovery (`home-content.tsx`), Feed (`feed-content.tsx`), and Favorites (`favorites-content.tsx`, via a frozen-ID-snapshot + batched `getEvents` call), plus every calendar view. Any change to `Query.events` or the `Event` type resolvers in `apps/backend/src/schema/resolvers.ts` **must** be evaluated for its per-row cost, because it multiplies across every consumer at once:
  - **Known N+1 hotspot (not yet fixed, tracked for follow-up):** `buildOptimizedDrizzleSelect` only maps requested GraphQL fields to real physical columns of the `events` table — it does **not** know about computed/virtual fields (`isFavorited`, `favoriteCount`, `isAddedToCalendar`, `isFromSubscribedAccount`, `schedules`), even though `fieldMap` already builds efficient `EXISTS`-subquery forms for several of these for `WHERE`-clause filtering. Because the shared `getEvents.graphql` document requests `favoriteCount`, `isFavorited`, and `schedules{...}` on every item, each of these currently falls through to its own per-parent `Event.*` field resolver and issues one DB round trip **per row, per field** — roughly `1 + 1 + 3N` queries for a page of `N` events (item list + count + schedules/favoriteCount/isFavorited per row), scaling up to `limit`'s max of 1000. Prefer batching these (reuse the existing `fieldMap` `EXISTS` expressions in the main flat select when the field is requested and `userId` is known, or a per-request DataLoader keyed by event id) over adding more per-row field resolvers.
  - **`totalCount` is computed unconditionally** on every `events` call via a second full query (with two left joins), even when a resolver could tell from `info` that the client didn't select it — the code has a `// Note:` comment acknowledging this. Do not add more unconditional secondary queries to this resolver; gate new ones on `info` field selection the way `buildOptimizedDrizzleSelect` already does for the item list.
  - When adding a new field to `Event` that Discovery/Feed/Favorites will request by default, default to a batched/joined implementation, not a new per-row field resolver — a per-row resolver here is a footgun that immediately multiplies by every list view in the app.
- **`Query.eventBySlug` (the event-detail page) is the second-highest-traffic endpoint** (added 2026-09-15, `bmad-agent-architect` review, see Architecture Spine AD-16). It's reached from every `EventCard` click-through across Discovery/Feed/Favorites/Calendar list views, direct/shared deep links, push-notification taps, and the "Correct Data" dialog surface that renders inside the event-detail modal. Any change to `eventBySlug`, `Event.instagramEmbed`, or the event-detail route components (`apps/web/src/app/[locale]/events/[slug]/page.tsx`, the intercepted `@modal/(.)events/[slug]/page.tsx`) must be evaluated for its effect on this page's critical rendering path:
  - AD-16 restructures `EventDetailWrapper.tsx` (the actual client-side data source for this route — `page.tsx` only fetches server-side inside `generateMetadata`, discarded after building the tab title) so primary event content is never gated on the Instagram oEmbed HTTP round trip: `instagramEmbed` is stripped out of `getEventBySlug.graphql`, and a second, independent React Query hook fires the new DB-free oEmbed query in parallel with `useGetEventBySlugQuery` — React Query dispatches independent hooks' requests concurrently on its own, no `Promise.all`/`race` needed. Do not re-nest `instagramEmbed` back into `getEventBySlug` or introduce a shared awaited gate that makes primary content wait on Instagram again.
  - New fields added to the event-detail query should default to the same optimized flat select `eventBySlug` already uses (matching `Query.events`' `buildOptimizedDrizzleSelect` precedent above), not a new per-field resolver making its own round trip.
...
- **Page Containers:** (added 2026-08-24, `bmad-correct-course`; extended 2026-08-24 same day) Every page **must** use the shared `PageContainer` primitive (`packages/ui/src/core/page-container.tsx`, Story 0.30) instead of a page-local `max-w-* mx-auto` wrapper — found as a copy-pasted `"p-4 sm:p-8 space-y-8 max-w-7xl mx-auto"` (and a separately-drifted `max-w-3xl`/`max-w-4xl` variant on settings pages) across 13 files before this rule existed; the duplication itself, not just any one width value, was the defect. Takes a `fullWidth?: boolean` prop (default `true`):
  - **`fullWidth={true}`** (Discovery, Favorites, Feed, Archive, the public account page, My Calendar, Manual Post Selection — not limited to this list): the container consumes available viewport width (`w-full`), no `max-width` cap, with a **responsive minimum width paired to breakpoint**: `min-w-[320px] sm:min-w-[640px] md:min-w-[768px] lg:min-w-[1024px] xl:min-w-[1280px]`. No-op in a normal unconstrained viewport — only matters when something else (a flex/shrink parent, or the embeddable widget's iframe host, PRD §3.14) tries to squeeze the container thinner than its own breakpoint should allow.
  - **`fullWidth={false}`** (Settings/table/form pages — API Keys, Subscribed Accounts, Notifications, Locations, Queue Status, Widgets — pages whose content is a list/form, not a card grid, and are never embedded, so they don't need `fullWidth={true}`'s narrow-host defense): `w-full max-w-5xl mx-auto lg:min-w-[768px]` — a single floor (not the 5-step schedule) since only one breakpoint transition matters here, capped at a **common `max-w-5xl`** (replacing the inconsistent `max-w-3xl`/`max-w-4xl` split) so a list/form doesn't stretch edge-to-edge on an ultrawide monitor.
...
- **Page Headers:** (added 2026-08-24, `bmad-correct-course`) Every page's title row **must** use the shared `PageHeader` primitive (`packages/ui/src/core/page-header.tsx`, Story 0.32) instead of a page-local `<h1>`+button row — found duplicated (with drifting heading styles and inconsistent mobile-label handling) across 18 files. Renders `flex justify-between items-center flex-wrap` (so on a narrow viewport a title and a wide action button wrap to their own lines instead of overflowing/squishing) with a standardized `text-3xl font-bold` `<h1>`; an optional `action` prop (`{ label, icon, onClick, disabled? }`) renders an icon+label button whose label is `hidden sm:inline` (icon-only below `sm:`) automatically; an optional `description` prop renders a `<p>` subtitle beneath the title.
