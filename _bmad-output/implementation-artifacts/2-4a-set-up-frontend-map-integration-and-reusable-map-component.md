...
4. **AC4 — Domain-agnostic prop API:** `MapView` accepts only generic props (`apiKey`, `center: Coordinates`, `zoom?`, `marker`, `onCoordinatesChange`, `mapStyle?`, `labels?`, `className?`, and — **[REVISED 2026-08-06, Sprint Change Proposal, `sprint-change-proposal-2026-08-06-map-picker-continuity.md`]** — `showZoomControl?`, `onViewStateChange?`, see AC12/AC13) — no FestGrid-specific business logic, enum imports, or knowledge of events/locations/subscriptions, so it can be reused by any future feature needing a map.
...
**[NEW 2026-08-06 — Sprint Change Proposal, `_bmad-output/planning-artifacts/sprint-change-proposal-2026-08-06-map-picker-continuity.md`]**

12. **AC12 — Zoom control affordance:** `MapView` accepts an optional `showZoomControl?: boolean` prop (default `true`). When `true`, a `maplibre-gl` `NavigationControl` configured with `showCompass: false` (zoom in/out buttons only, no compass) is added to the map's top-right corner on mount via `map.addControl(...)`. Unlike AC7's inherently pointer-only marker placement, these are real, keyboard-focusable DOM `<button>` elements rendered by `maplibre-gl` itself — a genuine (partial) accessibility improvement for zoom, though AC7's marker-placement limitation and its non-map-fallback requirement are unchanged. Passing `showZoomControl={false}` omits the control entirely, for any future consumer that wants to render its own zoom UI or none at all.
13. **AC13 — Camera-state change notifications:** `MapView` accepts an optional `onViewStateChange?: (state: { center: Coordinates; zoom: number }) => void` prop. When provided, it is called (via `maplibre-gl`'s `moveend`/`zoomend` events, not the high-frequency `move`/`zoom` events) whenever the map's camera settles after the user pans or zooms — by drag, scroll/pinch gesture, or the AC12 `NavigationControl` buttons — reporting the map's current `getCenter()`/`getZoom()`. This lets a consumer (e.g. Story 2.4) track the live camera position without polling, so it can persist "where the user left the map" independently of marker placement. Never called for camera moves `MapView` itself makes in response to a `center`/`zoom` prop change (no feedback loop) — only for user-driven camera movement.
...
- **Origin:** `_bmad-output/planning-artifacts/sprint-change-proposal-2026-08-06-map-picker-continuity.md`, itself triggered by a user-reported UX problem in Story 2.4's shipped "Pick on map" flow (map position/zoom/marker did not persist across the search / current-location / map-pick input modes, forcing users to re-orient the map from scratch each time). AC12/AC13/Task 11/Task 12 above are net-new, additive capability this amendment adds to `MapView` so Story 2.4's picker sheet can (a) offer a real zoom affordance and open at a closer default zoom, and (b) track the user's own pan/zoom so it can be restored the next time the sheet opens within the same form session.
- **Why amended in place, not split:** Both additions are small, backward-compatible, optional-prop extensions to a component that is `review` (not `done`) and has exactly one real consumer (Story 2.4) — there is no second consumer or cross-cutting mechanism here that would justify a separate story, unlike Story 0.22's genuinely cross-cutting split in the same-day `sprint-change-proposal-2026-08-06.md`.
- **File-location note:** This story's own Dev Notes (below) already document that the real, shipped `MapView` lives at `apps/web/src/components/ui/map.tsx`, not `packages/ui/src/core/map.tsx` as originally planned (a user-directed Gate 2 deviation made during Story 2.4's implementation). AC12/AC13/Task 11/Task 12 apply to that real file — do not resurrect the original `packages/ui` path.
