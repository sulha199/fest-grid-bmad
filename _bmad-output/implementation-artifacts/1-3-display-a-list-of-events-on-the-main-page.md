...
- **Gate 2 (UI Complexity & Reusability) — run fresh via subagent persona Freya:** **Gap found.** The infinite-scroll mechanism (IntersectionObserver sentinel + fetch-next-page + append-not-replace + unmount cleanup + error handling) is a cross-cutting primitive `project-context.md` already names explicitly (`useInfiniteScroll`, in its `packages/ui/src/hooks/` example) but no story owned building it. It is needed by at least two other stories beyond this one: Story 2.2 ("View favorited events" — `project-context.md`'s "List Navigation" rule and PRD §3.12 mandate infinite scroll for Favorites, but 2.2's current ACs say nothing about pagination) and Epic 5's manual post selection screens (Story 5.1/5.1a, FR52/FR53's "lazy loading of posts in tabs"). **Resolution:** split into a new prerequisite story, **Story 1.3c** (`1-3c-build-the-reusable-infinite-scroll-hook`), added to `epics.md` (positioned immediately before Story 1.3) and to `sprint-status.yaml` as a `backlog` entry — following the 1.3a/1.3b/1.6a precedent (a component/hook built once, homed off the first story that needs it, not promoted to Epic 0, mirroring how the cross-epic-reusable `EventCard` was scoped to `1.3b` rather than Epic 0). This story (1.3) now depends on 1.3c and only *consumes* the hook.
  - Other Gate 2 findings evaluated and resolved as **no gap**: the skeleton/loading grid is page-layout-specific and not reused elsewhere (build inline); the fuller "Event List View" component described in `design-artifacts/D-Design-System/01-event-list-view.md` (view toggles, Filter Hub, Quick Favorite, "Nearby" default) is correctly and already split across Stories 1.4 (search), 1.5 (filter), 2.1/2.1a (favorite), 2.5/2.5a (nearby/geo) — nothing here lacks a home.
- **Lightweight escape-hatch guard (no subagent):** Re-checked this story's specific scope against the swept report and Gate 2 findings above for anything neither anticipated. Nothing new found.

### Data Type Compatibility & Migration Requirements

- No database schema changes are required by this story itself.
- This story's `EventCard` rendering depends on the `Event.imageUrl` field, which is a hard dependency of Story 1.3a's AC6 (`posts` join), which is itself a hard dependency of Story 1.2a (`posts` table + `events.postId`, currently `backlog`). Until 1.2a and 1.3a AC6 both land, `imageUrl` will simply be absent/undefined for every card — `EventCard`'s existing fallback/placeholder (Story 1.3b AC3) already handles this gracefully; it is not a broken or degraded state, just the expected interim behavior. No workaround or stub is needed in this story.
...
- **File/path expectations (corrected against actual repo state — the prior draft of this story referenced a path that does not exist):**
  - Page route: `apps/web/src/app/[locale]/page.tsx` (**not** `apps/web/app/page.tsx`) — the app already uses `next-intl`'s `[locale]` segment routing, wired by Stories 0.6/0.7, and currently renders Story 0.3's theme/dialog verification content, which this story replaces.
  - `EventCard`: imported from `@festgrid/ui` (exported via `packages/ui/src/index.ts` once Story 1.3b lands — that barrel currently only exports `./core/app-shell`).
  - Shared infinite-scroll hook: imported from `@festgrid/ui`, sourced at `packages/ui/src/hooks/useInfiniteScroll.ts` (new, Story 1.3c).
  - Locale message files: `apps/web/locales/en.json`, `apps/web/locales/id.json` (already exist; currently contain only `HomePage.title` — confirmed by reading both files).
  - Generated GraphQL client code: `apps/web/src/generated/graphql.ts` (already exists; currently only exposes the `health` query — will gain `events`-related types/hooks once Story 1.3a's schema and this story's operation document land).
...
- **Reusable component/mechanism callouts:**
  ...
  - Reusable mechanism: infinite-scroll hook — scoped to the new Story 1.3c per the Gate 2 finding above; this story only consumes it.
...
- Recent commits (`cf0949a`, `07f8c16`, `5877f72`, `cf24aac`, `bf8ac8f`) are all `bmad-*` planning-artifact updates (story/epic-readiness documentation) — no in-flight application-code pattern exists yet for this specific page. The actual frontend pattern to follow is the already-landed scaffolding in `apps/web/src/app/[locale]/layout.tsx`/`page.tsx` (Stories 0.3/0.6/0.7/0.9): providers are already composed in the root layout; this story only adds page content inside that shell.

### Latest Tech Information

- `@tanstack/react-query` v5 (`^5.101.4`, already pinned in `apps/web/package.json`): `useInfiniteQuery` requires an explicit `initialPageParam` and a `getNextPageParam(lastPage, allPages)` callback (mandatory as of v5, unlike v4's implicit default) — map `getNextPageParam` to the next `offset` from Story 1.3a's offset/limit pagination shape (returning `undefined` once there are no more results), with pages accumulating in `data.pages` for the append-not-replace requirement (AC4).
- `graphql-codegen`'s `typescript-react-query` plugin (already a devDependency, `^7.0.5`) can generate a typed infinite-query hook directly from an `events` operation document once Story 1.3a's schema lands — prefer the generated hook over hand-writing `graphql-request` calls, consistent with `apps/web/src/generated/graphql.ts`'s existing pattern.
...
- [ ] **Hard dependency sequencing accepted:** Stories `1-3a-build-the-events-backend-graphql-api-layer` and `1-3b-build-the-reusable-eventcard-component` are both `ready-for-dev` but confirmed **not yet implemented** (backend GraphQL schema currently exposes only `health: Boolean!`; no `EventCard` component exists on disk). This story cannot functionally complete until both land. Confirm they will be implemented first (recommended), or explicitly accept starting this story's page-composition/test-scaffolding work against mocked data now, with real wiring following once 1.3a/1.3b ship.
- [ ] **New prerequisite accepted:** Story `1-3c-build-the-reusable-infinite-scroll-hook` was added (Gate 2 finding, see Dev Notes) as a `backlog` entry in `sprint-status.yaml`/`epics.md`, positioned before this story. Confirm it will be implemented first, or explicitly accept this story temporarily hand-rolling the scroll-trigger logic inline pending Story 1.3c (not recommended — Story 2.2/Epic 5 would then re-diverge from a shared implementation).
...
- [ ] Gate 1/2/3 findings acknowledged: Gate 1/3 cited from swept `epic-1-readiness.md` (no new gap for this story); Gate 2 run fresh, produced Story 1.3c (see Dev Notes).
...
- The events GraphQL resolver itself (Story 1.3a), the `EventCard` component itself (Story 1.3b), and the infinite-scroll hook itself (**Story 1.3c**, newly split off by this story's Gate 2 finding — see `epics.md`, `sprint-status.yaml`) — this story only consumes all three.
- Calendar/weekly view toggle (per `design-artifacts/D-Design-System/01-event-list-view.md`'s "View Toggles") — no Epic 1 story maps to it for the main discovery page; the PRD's calendar view is scoped to subscribed/personal events (Epic 2/3).
