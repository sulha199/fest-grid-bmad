...
- [x] **Task 1 — New shared `packages/ui/src/core/account-identity.ts` fallback helper (AC: 1, 2, 3, 9)**
  - [x] Add `export function getAccountIdentityLabel(displayName: string | null | undefined, username: string | null | undefined, fallback: string): string` implementing the same 3-way chain `AccountAvatar` already computes inline today (`displayName || (username ? '@'+username : fallback)`), parameterized by `fallback` so each caller supplies its own terminal default rather than hardcoding one shared string.
  - [x] This is a plain, dependency-free TS function — lives in `packages/ui/src/core/` (not `packages/domain`) because it is UI-presentation text formatting for two `packages/ui` components, not portable business logic; not added to `packages/ui/src/index.ts`'s public barrel since both current consumers (Task 2, Task 3) live inside `packages/ui` itself and import it by relative path — no cross-package consumer exists yet to justify a public export (see Dev Notes' Code Organization note).

- [x] **Task 2 — `packages/ui/src/core/account-avatar.tsx`: use the shared helper, widen `platform` nullability, lock in regression tests (AC: 5, 9)**
  - [x] Replace the inline `const altText = displayName || (username ? '@'+username : 'User avatar');` (current line 47) with `const altText = getAccountIdentityLabel(displayName, username, 'User avatar');` — same exact output for every existing input, verified by the existing 11-test suite passing unmodified.
  - [x] Widen `AccountAvatarProps.platform` from `platform?: string;` to `platform?: string | null;`, matching the existing nullability of `displayName`/`username` on the same interface (`string | null | undefined` throughout) — purely a type-signature widening, the `if (platform)` truthy check already handles `null` correctly with zero runtime change.
...
- [x] **Task 3 — `packages/ui/src/features/subscriptions/SubscribedAccountCard.tsx` + `.types.ts`: graceful-fallback rendering (AC: 1, 2, 3, 4, 9)**
  - [x] `SubscribedAccountCardProps`: widen `account.platform: string` → `platform?: string | null`, `account.displayName: string` → `displayName?: string | null`, `account.username: string` → `username?: string | null`, and the top-level `accountHref: string` → `accountHref?: string | null`. Leave `account.accountId: string` required and non-null — the one field this component cannot degrade without (needed to key the subscribe toggle and, per AC 8, gates whether the caller renders the card at all). Add `labels.unknownAccountLabel?: string` to the existing `labels` object type, alongside `subscribeLabel`/`unsubscribeLabel`/`checkingSubscriptionLabel`.
  - [x] Compute `const primaryLabel = getAccountIdentityLabel(account.displayName, account.username, labels?.unknownAccountLabel || 'Unknown account');` and render it in place of today's raw `{account.displayName}` (current line 48).
  - [x] Render the secondary `@{username}` line (current line 49) only when **both** `account.displayName` and `account.username` are truthy — i.e. `{account.displayName && account.username && (<span ...>@{account.username}</span>)}` — so when `displayName` is empty and `username` got promoted into `primaryLabel` as `@{username}` (AC 1), it is not also duplicated on the line below; and so an empty `username` alone never renders a bare `@`.
  - [x] Wrap the avatar+identity block in `<a href={accountHref} ...>` only when `accountHref` is truthy; when it is empty/missing, render the identical inner content (avatar, primary label, conditional secondary line) inside a plain `<div>` with the same layout classes minus the link-specific `href`/focus-visible-ring classes (there is nothing to navigate to or focus as a link). Keep the existing outer flex/gap layout classes on both branches so visual position is unchanged either way.
  - [x] Thread `platform={account.platform}` into `AccountAvatar` unchanged (already done since Story 0.i6f) — no change needed there beyond the type widening already covered by Task 2.

- [x] **Task 4 — `packages/ui/src/features/events/EventDetailView.tsx` + `.types.ts`: loosen the caller guard (AC: 6, 7, 8)**
  - [x] Change the render guard at current line 259 from `{accountId && accountPlatform && accountUsername && accountHref && (` to `{accountId && (` — the sole remaining precondition is `accountId` (AC 8); everything else degrades gracefully inside `SubscribedAccountCard` per Task 3.
  - [x] Drop the now-redundant `displayName: accountName || accountUsername` fallback computed at the call site (current line 265) — pass `displayName: accountName` raw and let `SubscribedAccountCard`'s own `getAccountIdentityLabel` chain (Task 3) do the one fallback computation, so the same fallback logic isn't duplicated in two places with two slightly different terminal behaviors.
  - [x] Add `const canActOnSubscription = !!(accountPlatform && accountUsername);` and change the two prop lines `onSubscribe={onSubscribeToAccount}` / `onUnsubscribe={onUnsubscribeFromAccount}` (current lines 270-271) to `onSubscribe={canActOnSubscription ? onSubscribeToAccount : undefined}` / `onUnsubscribe={canActOnSubscription ? onUnsubscribeFromAccount : undefined}` — this reuses `SubscribedAccountCard`'s existing "disable the toggle when the handler prop is absent" logic (already shipped, `SubscribedAccountCard.tsx` current lines 28-32) with zero new disabling logic needed, satisfying AC 7.
  - [x] Thread `unknownAccountLabel: labels.unknownAccountLabel` into the `labels={{ ... }}` object literal passed to `SubscribedAccountCard` (current lines 274-278).
  - [x] `EventDetailViewLabels` (`.types.ts`): add `unknownAccountLabel?: string;` alongside the other optional label fields (`scheduleCheckboxLabel?`, `checkingSubscriptionLabel?`, etc.) — optional, so `apps/web/src/features/events/mapper.test.ts`'s hand-written `LABELS` fixture keeps compiling unmodified, matching the exact precedent Story 0.i6f's Task 5 already established for this same interface.

- [x] **Task 5 — `apps/web/src/features/events/mapper.ts` + `apps/web/locales/{en,id}.json`: wire the new label (AC: 2)**
  - [x] Add `unknownAccountLabel: t('unknownAccountLabel')` to `useEventDetailViewLabels()`'s returned object, alongside the existing `subscribeButtonLabel`/`unsubscribeButtonLabel`/`checkingSubscriptionLabel` entries.
  - [x] `en.json`: add `"unknownAccountLabel": "Unknown account"` to the `EventDetailsPage` namespace, alongside the existing subscribe/unsubscribe strings.
  - [x] `id.json`: add the parallel Indonesian key (e.g. `"Akun tidak dikenal"`) in the same position — confirm final wording against this file's existing register rather than a literal machine translation.

- [x] **Task 6 — Tests (AC: all)**
  - [x] `packages/ui/src/features/subscriptions/SubscribedAccountCard.test.tsx`: add cases — empty `displayName` + present `username` → primary text is `@{username}`, no duplicate secondary line; empty `displayName` + empty `username` → primary text is `"Unknown account"` (default) and via a custom `labels.unknownAccountLabel` override; present `displayName` + empty `username` → secondary line is absent (no bare `@`); empty/missing `accountHref` → `screen.queryByRole('link')` is not present, but the avatar and identity text are still visible; existing "renders AccountAvatar...and link to accountHref" test (current line 23) continues to assert the `<a>` path since it still passes a valid `accountHref`.
  - [x] `packages/ui/src/core/account-avatar.test.tsx`: the 3 new empty-string cases from Task 2.
  ...
  - [x] `apps/web/src/features/events/mapper.test.ts`: confirm it still compiles unchanged (Task 4's optional-field choice for `unknownAccountLabel`) — no edit expected, re-run as part of verification.
...
Claude Sonnet 5 (bmad-dev-story, direct in-session implementation).

### Debug Log References

None — no failing test runs or blocking issues encountered. All test/lint/build runs passed on first attempt after implementation.

### Completion Notes List

- Implemented Tasks 1-6 exactly as scoped, no deviations. New `getAccountIdentityLabel(displayName, username, fallback)` helper in `packages/ui/src/core/account-identity.ts`, consumed by both `AccountAvatar` (alt text, fallback `'User avatar'`) and `SubscribedAccountCard` (primary label, fallback `labels?.unknownAccountLabel || 'Unknown account'`).
- `AccountAvatarProps.platform` widened to `string | null`; `SubscribedAccountCardProps.account.platform/displayName/username` and top-level `accountHref` all widened to `string | null | undefined`, `account.accountId` left required per AC 9.
- `SubscribedAccountCard`'s secondary `@{username}` line now renders only when both `displayName` and `username` are truthy (no duplicate, no bare `@`); the avatar+identity block renders inside a plain `<div>` (no `<a>`, no link semantics) when `accountHref` is falsy, otherwise unchanged `<a href>` wrapper — verified both branches keep identical inner content/layout classes.
- `EventDetailView.tsx`'s render guard narrowed from `accountId && accountPlatform && accountUsername && accountHref` to `accountId` alone (AC 8); new `canActOnSubscription = !!(accountPlatform && accountUsername)` gates `onSubscribe`/`onUnsubscribe` to `undefined` when false, reusing `SubscribedAccountCard`'s existing disabled-when-handler-absent logic (AC 6, 7) — no new disabling logic needed. Redundant `displayName: accountName || accountUsername` fallback at the call site dropped in favor of `SubscribedAccountCard`'s own fallback chain.
- Rewrote `EventDetailView.test.tsx`'s `'omits SubscribedAccountCard when essential account props are missing'` test: missing `accountId` still asserts the card is fully absent (AC 8); missing `accountPlatform`/`accountUsername` now assert the toggle is present but disabled (AC 6, 7), matching the new guard behavior.
- New `unknownAccountLabel` optional label threaded end-to-end: `SubscribedAccountCardProps.labels` → `EventDetailViewLabels` → `EventDetailView.tsx`'s labels object → `mapper.ts`'s `useEventDetailViewLabels()` → `en.json`/`id.json`'s `EventDetailsPage` namespace (`"Unknown account"` / `"Akun tidak dikenal"`). Confirmed `mapper.test.ts`'s hand-written `LABELS` fixture still compiles unmodified (field is optional).
- **Verification actually executed, not just planned** (per this workflow's standing rule): `pnpm --filter ui test` → 501/501 tests pass (full suite, not just touched files); `pnpm --filter web test -- run mapper` → 16/16 pass (2 test files); `pnpm lint` (repo root) → 6/6 tasks successful, zero errors, only pre-existing warnings in untouched files; `pnpm build` (repo root) → 7/7 tasks successful, all 38 static pages generated, zero errors. Sprint-status.yaml's `0-i6a-...` entry corrected from a stale `backlog` to `in-progress` (now `review`) at the start of this session — the entry had never been synced when the story file itself was created at `ready-for-dev` with a `baseline_commit` already set.
- Pre-Coding Approval Gate's explicit human approval was obtained via `AskUserQuestion` at the start of this `bmad-dev-story` session (the story file's own checklist still shows this item literally unchecked, since that section falls outside this workflow's permitted story-file edit areas — approval is recorded here instead, in Dev Agent Record, and in the Change Log below).

### File List

- `packages/ui/src/core/account-identity.ts` (new)
...
- `_bmad-output/implementation-artifacts/sprint-status.yaml` (modified — status correction + progression)
...
- 2026-09-16: `bmad-dev-story`: ready-for-dev -> review. Explicit human approval to begin implementation obtained via `AskUserQuestion` at session start (Pre-Coding Approval Gate's pending item). Corrected a stale `sprint-status.yaml` entry (`0-i6a-...` read `backlog` despite this story file already being `ready-for-dev` with a `baseline_commit` set — moved to `in-progress` at session start, now `review`). Implemented Tasks 1-6 exactly as scoped: new `getAccountIdentityLabel` helper (`packages/ui/src/core/account-identity.ts`) shared by `AccountAvatar` and `SubscribedAccountCard`; widened `AccountAvatarProps.platform` and `SubscribedAccountCardProps.account.platform/displayName/username`/`accountHref` to nullable/optional (AC 9); `SubscribedAccountCard` now renders a graceful primary-label fallback chain, suppresses the duplicate/bare-`@` secondary line, and swaps to a non-interactive `<div>` wrapper when `accountHref` is falsy (AC 1-4); `EventDetailView.tsx`'s render guard narrowed to `accountId` alone, with a new `canActOnSubscription` flag disabling (not hiding) the subscribe/unsubscribe toggle when `accountPlatform`/`accountUsername` is missing (AC 6-8); new `unknownAccountLabel` optional label wired end-to-end through `mapper.ts`/`en.json`/`id.json` (AC 2). Rewrote `EventDetailView.test.tsx`'s guard test per Task 6's exact spec. Verification actually executed (not just planned): `pnpm --filter ui test` 501/501 pass, `pnpm --filter web test -- run mapper` 16/16 pass, `pnpm lint` 6/6 tasks clean (zero errors, only pre-existing warnings), `pnpm build` 7/7 tasks clean (38/38 static pages). No deviations from the story's scoped Tasks/Dev Notes.
