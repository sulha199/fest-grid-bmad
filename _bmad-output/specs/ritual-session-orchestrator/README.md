...
**Rule: every env var any ritual or check needs comes from `<cwd>/.env`, loaded once at each entry point — never assume the invoking shell already has it.** Found and fixed 2026-09-12, starting from provider credentials (`DEEPINFRA_API_KEY` for `all-claude-deepseek-medium`) but the rule is general: all four entry points read `process.env` directly and were never loading `.env` themselves, so whether a run had what it needed depended entirely on whether the *calling shell* had already sourced the repo's root `.env`. The credential-missing case surfaced as a fast, silent-looking failure — `dispatch-ritual.ts on <story> did not complete successfully (exit 1) -- stopping before running checks`, no session file ever written under `mailbox/sessions/` — that superficially resembles the unrelated stall anomaly below (both look like "the run went nowhere") but is actually immediate and deterministic, not a 20-minute idle timeout. Fixed by a shared `load-env.ts` (`loadDotEnv(cwd)`, no new dependency — a small manual parser, since Node's own `--env-file` flag can't be threaded through `tsx`'s CLI easily), called from each of `dispatch-ritual.ts`, `run-ritual.ts`, `run-ritual-cline.ts`, and `run-act-with-checks.ts` right after their existing `--cwd` resolution, before any credential check. Existing `process.env` values always win over `.env`, so a real shell export still overrides a stale `.env` value.

Because `runScript`/`spawn` calls throughout this tool use `stdio: "inherit"` with no `env` override, a var loaded once at the top-level entry point propagates down through every descendant it spawns — `dispatch-ritual.ts` → `run-ritual-cline.ts`'s Cline child, and separately `run-act-with-checks.ts` → `run-check.ts` → `pnpm test`/`pnpm build`/`pnpm lint` → whatever those actually invoke (a real `postgres()` client connecting to `DATABASE_URL`, included). So this isn't just an LLM-provider-auth fix: it's what makes any DB-backed test or migration a dispatched ritual runs actually able to reach the same local Postgres the developer's own shell can, without the orchestrator needing its own separate DB-config plumbing. **Verified twice**: (1) with `DEEPINFRA_API_KEY` unset in the invoking shell, a direct `loadDotEnv("<repo-root>")` call correctly populated it from `.env` before the credential check that previously threw; (2) live end-to-end on Story 3.3e's `bmad-dev-story` — with no manual shell export at all, `run-act-with-checks.ts`'s post-act `pnpm test` reached local Postgres via `DATABASE_URL` (loaded the same way) and all 11 backend test tasks passed, confirming the inheritance chain works all the way from `.env` to a real DB connection several process-spawns downstream.
...
**Investigated and ruled out, 2026-09-12: standalone `cline` CLI's `--id <session-id>` resume is real but unusable headless.** The `@cline/sdk`-level gap above prompted a look at the separate `cline` CLI binary (v3.0.61, `~/.cline/data/sessions/`), which does advertise `--id <session-id>` ("Resume an existing session by ID") built on top of that same gap-having SDK. Live-tested end-to-end: a custom local tool via a stdio MCP server (`cline mcp add <name> --transport stdio --yes -- <cmd>`) works and correctly inherits the parent `cline` process's env vars (verified — a tool call echoed back an env var set only on the spawning process), so an MCP-based version of the mailbox's `ask_user_question` relay was viable in principle. But `cline --id <session-id>` itself hard-requires an interactive TTY no matter what: combined with `--json` it fails with "JSON output mode requires a prompt argument or piped stdin" even when a prompt *is* supplied (positional or piped, tested both ways, in both Git Bash and native PowerShell); without `--json` it fails outright with "interactive mode requires a TTY". Plain (non-resume) `--json` + a positional prompt works fine — the failure is specific to `--id`. Conclusion: real resume exists in this CLI, but only through the interactive TUI, not from any unattended/automated invocation — not usable as a resume mechanism for this orchestrator without a full TUI-automation harness (out of scope). The safe mitigation for a Cline child's transient-network death remains a whole-session retry (restart from scratch, not true resume), not `--id`.

**Implemented, 2026-09-12: whole-session retry with an idle watchdog, in `run-ritual-cline.ts`.** `runOneAttempt` now races the session's `ended` event against an idle (no-SDK-event) watchdog — default 20 minutes of total silence, generous enough not to trip on real tool use, and explicitly suppressed while `ask_user_question` is genuinely blocked on a relayed human answer (that wait is expected to be silent for as long as it takes a person to respond). `main` retries a failed attempt from scratch (fresh session, same prompt, up to `--max-attempts`, default 3, with doubling backoff) when `isTransientNetworkError` matches — which now covers both the original DeepInfra transport errors (`UND_ERR_SOCKET`/`ECONNRESET`/etc.) and the watchdog's own stall marker. `cline.dispose()` runs in a `finally` so a stalled or failed attempt never leaks a live instance/subscription into the next retry. **Verified live**: a smoke test with a short `--idle-timeout-ms` override reproduced a real stall (SDK went completely silent after "session started"), the watchdog fired at the expected time, was correctly classified as retryable, retried with backoff, stalled again identically, and correctly exhausted `--max-attempts` and failed cleanly (no hang, cleanup ran both times) — the full retry lifecycle, not just the happy path. Deliberately does NOT reset the working directory between retries (an earlier draft of this fix considered `git reset --hard`/`git clean -fd` as an "idempotency boundary" between attempts — rejected: it would discard any in-progress work outside this attempt's scope, and it directly defeats `bmad-dev-story`'s own resume design, which relies on already-written files and already-checked-off task checkboxes surviving a restart).

**Open, unresolved anomaly (2026-09-12): repeated real stalls with `gemini-3.5-flash-lite` via the `@cline/sdk` path specifically.** Three consecutive live sessions through `ClineCore`/`cline.start()` (i.e. `source: "core"` in `cline history`) stalled completely after "session started" with zero further SDK events, for a trivial one-line prompt that this exact model otherwise completes in 4-6 seconds via the plain `cline` CLI (`source: "cli"`). Not yet root-caused — plausible candidates include the test API key hitting a rate limit that manifests as silence rather than a clean error (this key was reused heavily across many diagnostic runs the same day), or something specific to the SDK invocation path vs. the CLI path. The watchdog above bounds the damage regardless of cause, but the underlying "why" is still open.

**Second open anomaly (2026-09-12): a Cline session can finish, print its final result, and still never let the process exit.** Story 3.4r's `bmad-dev-story` dispatch logged `session ended, reason=completed` and the full final-result markdown — genuine, correct completion, work already committed — but `run-ritual-cline.ts`'s process (and its `dispatch-ritual.ts`/`run-act-with-checks.ts` parents) then sat with zero further log output for 20+ minutes, never reaching `run-act-with-checks.ts`'s step 2 (lint/build/test). Distinct from the idle-watchdog anomaly above: that one triggers on silence *before* a session ever produces a result; this one hangs *after* a successful result, most likely inside `cline.dispose()`'s cleanup in the `finally` block, though not yet confirmed. The watchdog does not cover this case since SDK events (the final result) *did* arrive — only the process's own exit is stuck. Mitigation used: manually confirm the commit landed (`git log`) to verify no work was lost, `TaskStop` the monitor, then `taskkill /F` the orphaned node process tree (killing the top-level parent PID cascades to the children — no need to kill each one individually), and run `run-check.ts --kind lint|build|test` directly to complete the verification the automated pipeline never reached. Root cause still open; a real fix would need a hard timeout around the dispose/exit path itself, not just around the SDK-event stream. **Reproduced identically on Story 3.6l the very next dispatch** — same symptom (final result printed, then a silent, un-exiting process tree), same mitigation, so this is a repeatable pattern on this DeepInfra/Cline path, not a one-off.

**Local Postgres crashing under full-suite test load, 2026-09-12 — recurring, not a one-off.** Beyond the single unclean-stop incident logged above, the *same* `postgresql-x64-18` service was found stopped again during Story 3.6l's post-act test check, at almost exactly the same elapsed-time mark (~900s into a full `pnpm test` run) as it had failed during 3.4r's. Correlated with the C: drive sitting at 95% capacity (14GB free) at the time — a strong candidate cause, since Postgres crashing under WAL/checkpoint write pressure when disk space is critically low is a well-known failure mode, and both crashes happened under the heaviest sustained DB write load in the whole batch (the full monorepo suite, not an individual package's tests). After the user freed some space (95%→92%, 14GB→21GB) and restarted the service, a re-run still surfaced a *different* Postgres-side error mid-suite (`PostgresError: could not load (null)`, code `XX000`) before a clean restart resolved it — so 92% full may still not be enough headroom to fully rule this out for a future batch. Take-away: if a post-act test check fails with any Postgres-originated error (`ECONNREFUSED`, `could not open file`, `could not load`, or similar) partway through a long run, check both service status *and* disk space before assuming anything about the story under test — and don't assume one restart fixes it for the rest of a multi-story batch.

**Unrelated but compounding, 2026-09-12: local Postgres is a native Windows service (`postgresql-x64-18`), not Docker — and an unclean stop of it corrupted the local dev database.** Verifying 3.4r's test check surfaced `ECONNREFUSED` cascades (the service had stopped) and, after restarting it, a *second* failure mode: `could not open file "base/16388/<oid>": No such file or directory` — a genuine Postgres-level catalog/file mismatch, ultimately found on 17 of 29 tables in the local `festgrid` database, not just the one hit first. Restarting `postgresql-x64-18` needs an elevated shell (`Start-Service postgresql-x64-18`) that this orchestrator's own shell doesn't have — that step has to go back to the person driving the batch. The corruption itself (unrelated to Cline/the orchestrator, but blocking test verification all the same) was fixed by dropping and recreating the whole local database (`DROP DATABASE`/`CREATE DATABASE`, disposable dev data) and replaying `packages/database`'s `migrate.ts` + `seed.ts` from scratch, rather than patching each corrupted table by hand — verified after by querying every `public` table and confirming zero errors before re-running the check. Take-away for future batches: if a post-act test check fails with `ECONNREFUSED` or a Postgres `could not open file` error, this is an environment problem, not a code regression in whatever story is being verified — check the service and data directory health before assuming the dispatched story's implementation is at fault.
