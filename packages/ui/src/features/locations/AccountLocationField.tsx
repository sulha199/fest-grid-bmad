...
export function AccountLocationField({
  ...
  const editLabel = labels?.editLabel || "Edit Default Location";
  const pendingReviewLabel = labels?.pendingReviewLabel || "Pending Review";

  return (
    <div className="flex items-center gap-2 flex-wrap text-muted-foreground">
      <span>{displayAddress}</span>
      {isPendingReview && (
        <StatusBadge variant="pendingReview" label={pendingReviewLabel} />
      )}
      <button
        type="button"
        onClick={onEdit}
