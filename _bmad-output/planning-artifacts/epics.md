...
4.  **And** the first concrete adapter is a **real** Instagram adapter scraping through the `imginn.com` proxy/mirror (PRD §3.7's own named example) via HTTP fetch + HTML parsing (`cheerio`) — **explicitly unverified against the live site**, since this story was authored without live-browser access to confirm real selectors/anti-bot behavior against `imginn.com` (a direct fetch attempt during story creation returned `403 Forbidden`); manual validation against the live site and header/anti-bot tuning is expected as a near-term follow-up, not a blocker to merging this story's structurally-complete, fixture-tested implementation.
5.  **And** the second registered adapter, for Twitter/X, is an explicit **not-yet-implemented stub** (both interface methods throw a clear error) — no proxy/mirror site is named in the PRD for this platform, and building a second unverified live-scraping implementation in the same pass was deferred; it is registered now so the platform-slug/display-name registry (Story 3.3c) stays fully populated and Story 3.11's routing isn't blocked.
6.  **And**, distinct from the daily batch (AC1-3), when a user subscribes to an account and that subscription's lookup-or-create logic (Story 3.1a's `subscribeToAccount`) creates a **brand-new** `SocialMediaAccountProfile` row (i.e. no prior subscriber had ever subscribed to this account before — not triggered when subscribing to an already-known, already-profiled account), an on-demand scrape job for that one account is enqueued onto `ScrapingQueue` **asynchronously** — `subscribeToAccount` enqueues and returns immediately; it does not block on the scrape completing, and a failure to enqueue (e.g. a transient SQS error) is logged and swallowed rather than failing the subscribe mutation. This reuses the exact same per-account job-processing path as AC2/AC3, so a subscriber's first posts become available to Epic 5's (not-yet-built) manual post-selection screen without waiting for the next scheduled batch.
...
**Amendment (2026-08-08, added via `bmad-create-story` during this story's own creation):** The AC list above supersedes the original 2-bullet placeholder. Key decisions, made with the user via `AskUserQuestion` during this story's creation:
- **Cadence:** Story 0.14's `rate(6 hours)` placeholder is replaced with a daily schedule, both to reduce third-party scraping-API usage and because AC6's on-demand path now covers the "don't make a new subscriber wait" case that a tighter polling interval would otherwise have been compensating for.
- **Scraping technique (Instagram):** Real `cheerio`-based HTML scraping of `imginn.com` was chosen over (a) a headless-browser approach (Playwright + Chromium Lambda layer — rejected as a heavier, still-uncertain commitment: `imginn.com`'s `403` looks like it may be IP-reputation/datacenter blocking rather than a "not a real browser" check, which a headless Chromium running from a Lambda's AWS IP would not necessarily fix either) and (b) a managed scraping-API vendor (Bright Data / Apify — free tiers researched and viable, but rejected for *this* story to avoid introducing a new paid vendor credential/dependency before the DIY approach has been tried and evaluated against the live site). This is an explicit, accepted risk — see AC4.
- **Twitter/X:** Deliberately shipped as a registered-but-stub adapter (AC5) rather than a second unverified scraping implementation, since the PRD names no proxy/mirror for this platform.
- **On-demand scrape-on-subscribe (AC6):** New scope, not in the original placeholder AC. Retrofits Story 3.1a's already-`review`-status `subscribeToAccount` (`apps/backend/src/lib/subscriptions/subscribe-to-account.ts`) with a small, targeted addition (one call after the existing "created new profile" branch) — following the precedent of Story 3.3c's Task 6, which similarly made a small edit to already-shipped call sites rather than splitting a trivial retrofit into its own story. Confirmed async (queue-and-return, not blocking the mutation) — chosen because it reuses the existing SQS/Lambda plumbing exactly rather than adding a special-cased synchronous branch, and doesn't put a third-party scraping call on the critical path of the core subscribe mutation.
