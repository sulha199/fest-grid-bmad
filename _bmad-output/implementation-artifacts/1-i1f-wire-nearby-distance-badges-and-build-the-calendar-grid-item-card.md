...
8. **And** when no nearby filter is active (`selectedValue === 'off'`) and no other viewer coordinate is resolvable, `distanceKm` is `undefined` and the badge is simply omitted — never an error, never a placeholder. **REVISED during implementation (2026-09-20), user-directed (interim step ahead of Story 0.39):** the "else" branch is no longer categorically out of scope — `use-nearby-filter.ts` now silently falls back to the viewer's current-location coordinate when no filter is active, but **only** when the browser's Permissions API already reports geolocation as `granted` (from some earlier explicit action, e.g. this same filter's own "current location" option, or a location-picker form elsewhere in the app). This is a deliberately narrow interim mechanism: it never triggers a new browser permission prompt on its own — if permission is `prompt`/`denied`, or the Permissions API is unsupported, the badge stays omitted exactly as before. **Story 0.39's own scope is unchanged and still needed**: proactively *asking* a user who has never granted geolocation (via its planned ambient consent banner, shared with the PWA-install banner slot) remains out of scope here — this revision only covers reusing consent a user has *already* given.
...
- [x] **Task 8 — Full verification (all ACs)**
  ...
  - [x] 8.2 `packages/ui`'s full Vitest suite passes (545/545 tests, 52 files, after the 2026-09-20 revision added 7 — see Change Log) — no regression in `EventCard`/`WeeklyCalendarView`/`useWeeklyCalendarController`/`useCurrentLocationCapture` or any other suite.
  - [x] 8.3 `apps/web`'s full Vitest suite passes (395/395 tests, 60 files, after the 2026-09-20 revision added 4), including `nearby.test.tsx` (`home-content`/`use-nearby-filter` coverage, including the new ambient-fallback tests) and `CalendarView.test.tsx`.
...
| No nearby filter active (`off`), geolocation permission already `granted` | **REVISED 2026-09-20:** `activeFilterCoord` = the silently-captured ambient coordinate (`useCurrentLocationCapture().captureIfPermissionGranted()`, session-gated) | same value, threaded as `viewerCoord` (shares the one `useNearbyFilter()` instance) |
| No nearby filter active (`off`), permission `prompt`/`denied`/unsupported, or anonymous | `activeFilterCoord` = `undefined` → badge omitted (unchanged) | same → `distanceKm` omitted |
...
- **Verification Plan:** Task 8's full test/lint/typecheck pass across `packages/domain` (323/323 tests), `packages/ui` (545/545 tests, post-revision), and `apps/web` (395/395 tests, post-revision) — all green; `pnpm --filter web build` succeeds; `pnpm lint` at the repo root is clean (0 errors); `tsc --noEmit` in `apps/web` shows only pre-existing baseline errors, none in any file this story touches; GraphQL codegen diff confirmed additive-only; `feed-content.tsx`/`favorites-content.tsx` confirmed untouched via `git diff`.
...
- **Story 0.39** — the ambient consent-**banner** that proactively asks a user who has never granted geolocation (coordinated with the PWA-install banner slot, Story 0.42). **REVISED 2026-09-20:** this story now covers a narrower interim mechanism — silently reusing an *already-granted* geolocation permission when no filter is active (see AC7-8) — but that is not a substitute for 0.39: a user who has never been asked still sees the badge omitted with no filter active, exactly as before this revision, until 0.39's banner ships.
...
- [x] All Task 8 tests passing (`packages/domain` 100% coverage on new file, 323/323 suite; `packages/ui` 545/545; `apps/web` 395/395).
