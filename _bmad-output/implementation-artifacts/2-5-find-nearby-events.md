...
4. ~~**And (auto-default on first visit)** if I am authenticated and have at least one saved location, the Discovery page automatically applies the nearby filter on load using my earliest-created saved location (`createdAt` ascending — the implicit "primary", no new schema needed) and that location's own configured `radius`, without requiring me to manually open the filter — matching `D-Design-System/01-event-list-view.md`'s "Defaults to Nearby" behavior.~~ **SUPERSEDED (2026-08-29, `spec-ux-rework2-batch.md`)** — see note below.
5. ~~**And (no-saved-location fallback)** if I am authenticated but have zero saved locations, on first visit the app attempts to capture my current browser location (`navigator.geolocation`, reusing Story 2.4's capture hook) and, if granted, filters nearby using those ad-hoc coordinates via Story 2.5a's AC1a `{ latitude, longitude, radiusKm }` shape with a default 5km radius. This attempt is silent/backgrounded — no error banner is shown if it fails; the list simply falls back to AC6's unfiltered behavior.~~ **SUPERSEDED (2026-08-29, `spec-ux-rework2-batch.md`)** — see note below.

> **Amendment (2026-08-29, `bmad-quick-dev` / `spec-ux-rework2-batch.md`):** AC4 and AC5's auto-apply behavior (first-visit saved-location default and the no-saved-location geolocation prompt) was removed at the user's explicit request ("disable auto redirect to nearby filter" on the Discovery page) after it was found to be surprising/unwanted in practice. The Nearby filter is now **opt-in only**: it activates solely via explicit user action (opening the FilterHub control) or an existing `nearby` URL param already present on load — never automatically. AC1-AC3, AC6 (unfiltered fallback), AC7 (unauthenticated no-op), AC9-AC13 (radius pre-fill, FilterHub placement, i18n, a11y, compact presentation) are unaffected and remain implemented as specified. AC8's "does not re-trigger" clause is now moot (nothing auto-triggers to begin with) but its underlying guarantee — an explicit selection persists via URL state across reloads — still holds. Implementation: `apps/web/src/app/[locale]/use-nearby-filter.ts` (commit `5d05993`/`125444b`).
...
- [x] AC1-AC3, AC6-AC12 satisfied (confirmed already implemented via direct code inspection, 2026-08-25); AC4/AC5 superseded 2026-08-29 (see amendment note above) — the Nearby filter is opt-in only, no auto-apply.
...
**2026-08-29 (`spec-ux-rework2-batch.md`):** AC4/AC5 superseded per explicit user decision — removed the first-visit auto-apply (saved-location default) and no-saved-location geolocation-prompt fallback from `use-nearby-filter.ts`. Surfaced by adversarial review of the ux-rework2 batch (the change had deleted behavior this story's own Definition of Done claimed as done, with no accompanying story amendment); user chose to keep the removal and amend this story rather than revert. See the amendment note under AC4/AC5 above.
