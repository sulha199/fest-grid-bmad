...
- **Known follow-up, not fixed here:** mounting an icon-bearing `@festgrid/ui` component (e.g. the story's originally-named `EventCardDateBox`) still hits the `lucide-react` dual-package/Playwright-transform interop bug documented in the Debug Log. Whichever future story needs that (most likely BUG-040's fix) will need to resolve it then.
