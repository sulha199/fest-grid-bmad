...
**As a** developer,
**I want** a shared, reusable way for a page to obtain the viewer's current geographic coordinate passively (not only inside an explicit "use my location" click), with an explicit, designed policy for permission-prompt timing and caching,
**So that** "nearby" features (starting with the distance badge on `EventCard`/the desktop calendar grid card, Story 1.i1f) can show a real distance when the viewer has no active location filter selected, without every feature re-deciding its own consent/caching approach.

**Acceptance Criteria:**

*   **Given** no global/app-shell "viewer location" context, provider, or persisted state exists anywhere in this codebase today (confirmed: only an imperative, one-shot `useCurrentLocationCapture()` wrapper around `navigator.geolocation.getCurrentPosition`, called exclusively from explicit user-initiated actions — the nearby-filter's own "current location" mode, and two location-picker forms),
*   **When** this story ships,
*   **Then** a new shared mechanism (e.g. a `ViewerLocationProvider`/`useViewerLocation()` pair, exact shape left to the implementation story) exists that a page can consume to obtain "the viewer's current coordinate, if available and permitted" without itself deciding consent timing.
*   **And** the mechanism has an explicit, documented policy for **when** it may prompt for the browser geolocation permission (e.g. only after some existing signal of intent — never an unconditional prompt on first paint of every route) and for **caching/persistence** (e.g. in-memory for the session, or a short-TTL store) so it does not re-prompt or re-fetch on every render/remount.
*   **And** a consumer that receives no coordinate (permission denied, unsupported, not yet granted) degrades to "no viewer coordinate" with no error thrown — callers (e.g. Story 1.i1f's nearby badge) already treat a missing coordinate as "omit the badge," not a failure state.
*   **And** this story does not change the "active filter location" branch of the distance priority rule (Architecture Spine AD-22) at all — that branch is already implemented by Story 1.i1f via `useNearbyFilter`; this story only supplies the "else" branch's coordinate source.
...
### Story 0.40: Wire up packages/ui's standard ESLint configuration
...
*   **Given** AD-17 Rule 1, **when** `apps/backend/src/schema/resolvers.ts`'s `events` resolver builds its `db.select({...})`, **then** `packages/graphql-select/optimized-select.ts`'s `buildOptimizedDrizzleSelect` gains an optional `virtualFields: Record<string, SQL>` parameter, and the resolver passes its existing `fieldMap`'s `isFavorited`/`isAddedToCalendar` `EXISTS` expressions (today wired only into `buildDrizzleWhere`) plus a new `favoriteCount` entry (`(SELECT count(*) FROM favorites WHERE favorites.event_id = events.id AND favorites.deleted_at IS NULL)`) as `virtualFields`, each included only when the corresponding GraphQL field is actually requested per `info` — the same signal `buildOptimizedDrizzleSelect` already uses for `items`.
*   **And** `Event.isFavorited`/`Event.favoriteCount`/`Event.isAddedToCalendar` field resolvers become passthroughs reading the pre-populated parent value, falling back to today's per-row query only when the value is absent (a defensive fallback for a caller that somehow reaches the field resolver without pre-population — never the expected path once this ships).
*   **Given** AD-17 Rule 2, **when** the `events` resolver has fetched its page of parent rows and `schedules` (or a nested `Schedule` field, per AD-17 Rule 3) was requested, **then** it issues one additional `db.select({...buildOptimizedDrizzleSelect(schedules, info, {path: 'schedules', virtualFields: {...}})}).from(schedules).where(inArray(schedules.eventId, ids))` query, groups the results by `eventId` in JS, and attaches them to each parent row as `item.schedules` before returning. `Event.schedules` becomes a passthrough reading `parent.schedules`, falling back to its current per-row query only when absent.
*   **Given** FIND-027, **when** the `events` resolver's `totalCount` second query (`resolvers.ts` ~3165-3170) runs, **then** it is gated on `info`'s field selection actually requesting `totalCount`, the same technique `buildOptimizedDrizzleSelect` already applies to `items`.
*   **Given** BUG-034, **when** the migration for this story runs, **then** a partial index `CREATE INDEX idx_favorites_event_id ON favorites (event_id) WHERE deleted_at IS NULL;` is added via a Drizzle-kit-generated migration, hand-edited to include the `WHERE` clause per AD-8 Rule 3's documented drizzle-kit `WHERE`-clause-dropping limitation (matching `idx_favorites_active`'s existing hand-edit precedent) — this index ships in this story because Rule 1's per-row correlated subqueries are only viable with an `eventId`-leading index; without it they would force a sequential scan of `favorites` once per output row inside a single query execution, which is worse than today's separate per-row queries, not a fix.
*   **Given** FIND-028, **when** `apps/web/src/app/[locale]/home-content.tsx`, `feed/feed-content.tsx`, and `favorites/favorites-content.tsx`'s `getEvents`-consuming query hooks are configured, **then** each is given `staleTime: 30_000` (30s), cutting refetch volume on remount/window-refocus without materially staling Discovery/Feed/Favorites data.
*   **And** existing DSL/resolver behavior is unchanged and regression-verified: `eq`/`ne`/`contains`/`in`/`notIn` operators and the existing `fieldMap` entries continue to behave exactly as before (existing `resolvers`/`drizzle-where` tests pass unmodified).
*   **And** a query-count integration test asserts that fetching a page of N events (N > 1) now costs a small constant number of queries, not `O(N)` — the concrete regression target for the PRD's Performance NFR's "explicit query-count/cost check" requirement.
*   **And** this story does not change the `event(id)`/`eventBySlug` resolvers — those are Story 1.6c's scope, which reuses this story's `virtualFields`/batched-schedules mechanism rather than reimplementing it.

**Note:** This story exists because of a `bmad-agent-architect` performance audit (2026-09-15) of `Query.events`, the app's highest-traffic query per the PRD's Performance NFR (added 2026-09-15, same session). The audit found BUG-030 (the N+1 itself), FIND-027 (an ungated `totalCount` query), BUG-034 (a missing index that blocks BUG-030's own fix from being viable), and FIND-028 (missing `staleTime`) — batched into one story per AD-17 Rule 5's explicit grouping, since all four touch the same resolver/query surface and FIND-027/FIND-028 are "trivial to include, not worth a separate story" in AD-17's own words. The batching mechanism itself (`virtualFields` extending `buildOptimizedDrizzleSelect`, reusing `fieldMap` rather than introducing DataLoader) was decided by a dedicated `bmad-architecture` pass — see Architecture Spine **AD-17**. Positioned as the last lettered suffix in the 1.3 family (after 1.3i), immediately before Story 1.3 — discovered after every other 1.3-family split, following this epic's "discovery order, not alphabetical" placement convention (per Story 1.3g's own note). Registered via `bmad-correct-course` as **CC-020** (`sprint-change-proposal-2026-09-15-getevents-eventbyslug-perf-hardening.md`).

**Depends on:** Story 1.3a (the `events` resolver this story amends), Story 0.8 (`buildOptimizedDrizzleSelect`, extended here).
...
*   **Given** `packages/domain/src/events/buildEventsQueryCondition.ts`'s module-local `getDays(fromStr, toStr, dow: DayOfWeek | string)`, **when** generalized per Architecture Spine AD-19 Rule 1, **then** it is `export`ed with signature `getDays(fromStr: string, toStr: string, dow: DayOfWeek[]): string[]`, matching any weekday in the array (a union), with the existing `EventFilterInput.dayOfWeek` call site wrapped as a 1-element array and zero behavior change to today's single-weekday filter matching.
*   **And**, per AD-19 Rule 2, an explicit `Record<GqlDayOfWeek, DomainDayOfWeek>` mapping (`apps/web/src/lib/day-of-week-mapping.ts`) converts the GraphQL-generated `DayOfWeek` enum (`apps/web/src/generated/graphql.ts`) into domain's own `DayOfWeek` enum before any shared weekday-matching function is called — never an implicit string-value assumption.
*   **Given** `Schedule.applicableDaysOfWeek?: DayOfWeek[]` is PRD-decided (§4.4, BUG-026) but absent from both the DB schema and the GraphQL schema today, **when** this story ships, **then** `packages/database/schema.ts`'s `schedules` table gains a nullable `applicableDaysOfWeek: text().array()` column (matching the existing `events.types`/`events.categories` convention) via a Drizzle-kit-generated migration, and `Schedule.applicableDaysOfWeek: [DayOfWeek!]` is added to `apps/backend/src/schema/events.graphql`, resolved with zero new resolver code via the existing `buildOptimizedDrizzleSelect` passthrough already used by `Event.schedules` — the same additive-field class as Story 0.37's `Event.links`. AI-extraction population and `buildEventsQueryCondition.ts`'s filter-*matching*-correctness fix remain explicitly out of scope (BUG-026's own still-open items).
*   **Given** `WeeklyCalendarView.tsx`'s `dayBuckets` computation (today: a schedule occupies every day in `[eventStartDate, eventEndDate]`), **when** a schedule's `applicableDaysOfWeek` is set and non-empty, **then** only days whose weekday is in that list count as occurrences (via the shared `getDays` utility); a schedule with the field unset/empty behaves exactly as today.
*   **And** `isFirstSegment`/`isLastSegment` generalize from "equals the schedule's overall start/end" to "the immediately adjacent calendar day (±1), including days outside the visible week, is not also an occurrence" — collapsing calendar-adjacent occurrence days into one run (rendered identically to today's multi-day span, "Day X of N" keyed off the run's own length, never the schedule's overall span) and treating non-adjacent occurrences as isolated single-day segments.
*   **And** a new icon-only `event_card_repeat_badge` (lucide `Repeat`, `DESIGN.md` token) renders wherever a card's schedule has `applicableDaysOfWeek` set and non-empty, across all 3 card families: masonry `EventCard` (`badge_row`, order status → repeat → nearby), and `WeeklyCalendarView`'s `CalendarCard` in both `variant='list'` and `variant='grid'` (leading-icon-then-title row, alongside `isFavorited`/`isAddedToCalendar`) — built as one shared `EventCardRepeatBadge` primitive in `EventCardMediaPrimitives.tsx`, not three duplicates. It always carries an `aria-label` listing the schedule's translated matching weekdays (new `DayOfWeek` next-intl namespace, en/id) and shows the same content in a tooltip triggered by each card's own already-interactive root (never an independently-focusable nested trigger, avoiding the interactive-nesting hazard Stories 1.i1d/1.i1e already had to solve for the favorite badge).
*   **And** existing behavior for schedules without `applicableDaysOfWeek` is unchanged and regression-verified across `buildEventsQueryCondition.test.ts`, `WeeklyCalendarView.test.tsx`, `EventCard.test.tsx`, and `EventListView.test.tsx`.
...
**As a** developer,
**I want** the event-detail page's `eventBySlug`/`event` resolver to reuse Story 1.3j's batched-schedules mechanism for `Schedule.isAddedToCalendar`, its server/client double-fetch eliminated via cache hydration, and its subscriptions query gated on actually needing it,
**So that** opening an event's detail page costs one resolver fan-out instead of two, and does not issue a schedule-count-sized number of extra queries or an unconditional subscriptions call.

**Acceptance Criteria:**

*   **Given** AD-17 Rule 3 (BUG-033), **when** `event`/`eventBySlug` resolves `schedules`, **then** it reuses Story 1.3j's Rule 2 batched `IN (...)` query verbatim — the same code path, not a separate implementation — whose `virtualFields` map additionally includes an `isAddedToCalendar` entry (`exists(db.select(...).from(calendarAdditions).where(and(eq(calendarAdditions.userId, userId), eq(calendarAdditions.scheduleId, schedules.id), activeOnly(calendarAdditions))))`, gated on `userId` being known). `Schedule.isAddedToCalendar` becomes a passthrough reading `parent.isAddedToCalendar`, falling back to today's per-schedule query only when absent. This applies identically whether `event`/`eventBySlug` returns one row — an `IN (...)` query over a one-element id array is the same query shape as Story 1.3j's page case, no resolver-arity special-casing needed.
*   **Given** BUG-035, and the confirmed fix direction (HydrationBoundary + `dehydrate` cache seeding — the codebase's first use of this pattern; verified no `React.cache`/`unstable_cache`/`HydrationBoundary` usage exists anywhere in `apps/web/src` today), **when** `apps/web/src/app/[locale]/events/[slug]/page.tsx` and the intercepted `@modal/(.)events/[slug]/page.tsx` run their existing server-side `generateMetadata` fetch (`graphqlClient.request(GetEventBySlugDocument, {slug})`), **then** that same full-document fetch result is dehydrated (`dehydrate(queryClient)`) into a `HydrationBoundary` wrapping `EventDetailWrapper`, keyed to match `useGetEventBySlugQuery`'s own React Query key, so `EventDetailWrapper.tsx`'s client-side query hydrates from that cache on initial render instead of issuing its own network request. `generateMetadata` itself is unchanged in scope (still reads only `eventName`/`description` from the result) — the full document is no longer wasted work once it is reused for hydration.
*   **And** `EventDetailWrapper.tsx:317-326`'s `router.prefetch()` for the adjacent next/prev event is unaffected by this change — out of scope, a separate speculative prefetch, not part of BUG-035's doubled-fetch defect.
*   **And** a regression test confirms that a first page load of `/events/[slug]` (and the intercepted modal route) issues exactly one `GetEventBySlugDocument` network request to the backend, not two — verified via a request-count assertion in an integration/E2E test around the page route.
*   **Given** FIND-030, **when** `EventDetailWrapper.tsx:58-64` calls `useGetMySubscriptionsQuery`, **then** its `enabled` condition is narrowed from `!!session` alone to `!!session && !!event.sourceSocialMediaAccountProfile` — matching FIND-030's own stated fix direction (gate on the event actually having a linked source account) over lifting it to shell level, since — unlike `useMeQuery` — no other route needs this query warmed at shell level, and gating removes the call entirely for the common case of an event with no linked account.
*   **And** existing resolver behavior for `event`/`eventBySlug` is otherwise unchanged and regression-verified: `sourceSocialMediaAccountProfile`, `isFavorited`, `favoriteCount`, `isHiddenForCurrentUser` continue to resolve exactly as before.

**Note:** This story exists because of the same `bmad-agent-architect` audit (2026-09-15) that produced Story 1.3j, scoped to the `eventBySlug`/event-detail-page surface rather than the list surface. AD-17 Rule 5 sequences this story after 1.3j because Rule 3's `isAddedToCalendar` batching reuses Rule 2's mechanism verbatim, and keeps it a separate story because it targets a different route (event-detail vs. the three list views) and bundles a materially different fix category (frontend SSR-hydration caching, not DB-layer batching) that doesn't belong in 1.3j's DB-layer change set. BUG-035's fix direction — left open by AD-17 itself ("not decided by this AD... left for story-drafting to resolve") — was resolved via `bmad-correct-course` in favor of HydrationBoundary + `dehydrate` seeding over the narrower "cheapen `generateMetadata`'s own query" alternative, since it actually eliminates the duplicate fetch rather than only cheapening one side of it; this introduces a new SSR-hydration pattern to `apps/web` with no prior precedent — recommend `project-context.md` documents it as the canonical convention for future SSR+client-fetch pages once this story ships (a follow-up documentation task, not this story's own scope). FIND-030 is folded in as an additional AC rather than a third story — it shares this story's exact page/component (`EventDetailWrapper.tsx`) and is small, but is a different mechanism (a missing gate condition, not DB batching) from BUG-033/BUG-035, so it does not warrant its own story. Positioned as the last lettered suffix in the 1.6 family (after 1.6b), immediately before Story 1.6 — discovered after 1.6a/1.6b, following this epic's discovery-order placement convention. Registered via `bmad-correct-course` as **CC-020** (`sprint-change-proposal-2026-09-15-getevents-eventbyslug-perf-hardening.md`).
...
6.  **Given** the top-of-page Add-to-Calendar icon/button and the bulk multi-select `AddToCalendarDialog` UI it opens, **when** this story ships, **then** the standalone top-level button is removed, but the bulk dialog itself, its `isDialogOpen`/`triggerRef` state, and its `handleTriggerClick` handler (including its existing unauthenticated shortcut) are all **retained, unchanged** — the trigger is relocated into the existing overflow "more actions" menu as a new "Add to Calendar" entry, gated on `onAddToCalendar` being provided, alongside Correct Data/Report. This keeps a working calendar-add entry point on the page until Story 1.6e's per-schedule icon ships. The underlying `onAddToCalendar(selectedScheduleIds: string[])` prop and `EventDetailWrapper.tsx`'s `handleAddToCalendar` mutation-orchestration function are unaffected either way.
7.  **And** all new/changed user-facing text (the published-date prefix, and the "Event categories and types" `aria-label` on the badge list — today hardcoded English, not sourced from `labels`) is resolved through `next-intl` via the existing `EventDetailViewLabels` prop contract, matching every other string in this component.

**Note:** Covers IDEA-030 items 1, 2, 4, 5, 6, 7 in one story, following this project's precedent for batching several small same-file, same-component UI amendments (Story 1.3b's TILL-badge amendment; Story 1.6e's own three-item batch). **Item 3 (hashtags at the bottom of event-main-content, clicking to Discovery) is explicitly out of scope** — it depends on hashtag-persistence data that does not exist yet (BUG-032, unresolved, in a separate not-yet-started backlog cluster) — carved to a new child backlog row, `IDEA-037` (parent: `IDEA-030`, blocked on `BUG-032`), rather than absorbed or silently dropped. Gates 1/2/3 (`story-split-gate.md`) ran fresh via subagent dispatch — `epic-1-readiness.md` is `swept: true` but its `stories_covered` list predates Stories 1.6b-e and this one, so it was not cited (same reasoning as Story 0.36's fresh-Gate precedent). All three returned **no gap**: Gate 1 confirmed the new `Event.publishedAt` field is the same additive-field-on-an-already-joined-query class as Story 0.37's `Event.links`, not a new API surface; Gate 2 confirmed no item clears the reuse bar for extraction (the clickable-badges behavior is flagged as "watch, not split" — reusable-badge extraction becomes worth revisiting only if a future story adds the same click-through to `EventCard`/`CalendarCard`'s own badge row) and logged, informationally, that neither `DESIGN.md` nor `EXPERIENCE.md` covers event-detail-page layout order, title size, favorite-icon placement, badge interactivity, or the add-to-calendar affordance (matching the zero-coverage precedent already found for Stories 0.37/1.i1e); Gate 3 confirmed every mechanism this story consumes (i18n, `PlatformIcon`, the `nuqs` filter convention, the flat-select resolver pattern) is already built by a prior story. Two genuine design tradeoffs with no DESIGN.md/EXPERIENCE.md answer and no mechanical resolution were raised to the user via `AskUserQuestion`; this session's own in-conversation call reported no answer (a repeat of the intermittent non-response already documented on Stories 0.34/0.37), but per the Story 0.34 precedent that report was independently verified against the primary mailbox record rather than trusted at face value, which confirmed genuine answers: (1) the bulk `AddToCalendarDialog` is relocated into the "more actions" overflow menu (not removed) — keeping a working calendar-add entry point on this page until Story 1.6e ships its per-schedule replacement; (2) badge-click navigation is a clean single-facet param reset (`/?categories=X`), not a merge into whatever params the referring list carried.
...
**So that** "nearby" distance is computed the same way everywhere it's shown, and the desktop calendar grid gains the same thumbnail/badge treatment as its sibling card families (IDEA-026).

**Acceptance Criteria:**

*   **Given** the existing SQL haversine expression already used server-side by `withinRadius` (`packages/graphql-select/drizzle-where.ts`, spherical law of cosines, 6371km radius, clamped),
*   **When** `computeDistanceKm(viewerCoord, targetCoord)` (`packages/domain/src/query/`) is implemented,
*   **Then** it mirrors that exact formula/constants in pure TypeScript, with 100% unit test coverage.
*   **And** `EventCard.tsx`'s nearby-badge gate is corrected from its currently-shipped `distanceKm <= 5` to DESIGN.md's `< 8` threshold, and actually receives a real, non-`undefined` `distanceKm` from the Discovery page (`home-content.tsx`) for the first time — computed from the display schedule's coordinates and the active nearby-filter's resolved coordinate (fixed in this story to also cover a *saved*-location filter, not only the ad-hoc "current location" mode).
*   **And** when no nearby filter is active, the badge is simply omitted (not an error) — the "else, viewer's current location" branch of the priority rule is explicitly out of scope here (Gate 3, Story 0.39).
*   **And** `feed-content.tsx`/`favorites-content.tsx` remain unwired (pre-existing `BUG-025`: no nearby-filter plumbing exists on those pages at all) — not a new gap introduced by this story.
*   **And** `getEventsForCalendar`/`getEventsForMyCalendar` (`apps/web/src/features/events/queries.graphql`) select `locationDetails { coordinates { lat lng } }` on `Schedule`, matching `getEvents`' existing selection, and `WeeklyCalendarViewScheduleShape` gains `distanceKm?: number`, computed by `useWeeklyCalendarController` from that data plus `CalendarView.tsx`'s own nearby-filter coordinate (home page's calendar view only, same non-scope-creep rule as above).
*   **And** a new `EventCardCalendarGridItem` primitive (`packages/ui/src/features/events/`) implements DESIGN.md's `event_card_calendar_grid_item` tokens: a with-image composition (multi-day schedules with a non-errored image only — never a reserved slot — thumbnail, title/venue column with no-truncate title and 2-line-clamp venue, and a right-side stack holding the large favorite control and the nearby badge, `items-center` aligned, no status badge), and a structurally distinct no-image composition (single-day events, or a multi-day event whose image errored — two stacked rows: title+favorite, then venue+nearby badge).
*   **And** this primitive is built and unit/component-tested standalone — it is **not** wired into `WeeklyCalendarView.tsx`'s actual `variant='grid'` render path yet (which keeps rendering today's plain-text `CalendarCard` unchanged); adoption is deferred to Stories 1.i1g/1.i1h, matching this epic's own established build-primitive-then-adopt sequencing (1.i1a → 1.i1c/d/e).
...
*   **Given** today's `WeeklyCalendarView.tsx` has no true cross-column spanning mechanism — a multi-day schedule currently renders as N independent `CalendarCard` instances, one per overlapping day, cosmetically joined via `multiDayRoundingClass`'s corner-rounding suppression at touching edges, not one spanning element,
*   **When** this story ships,
*   **Then** a multi-day schedule renders as exactly one `EventCardCalendarGridItem` (with-image composition) positioned to span its `N` day-columns (width `N * day_cell_width`) directly in `grid_weekly`, built via `Story 1.i1f`'s primitive.
*   **And** single-day rendering in `grid_weekly`/`day_cell` is completely unchanged by this story (still today's plain-text `CalendarCard`, still capped at `event_rendering.discovery_view.max_events_per_day` (5)) — single-day adoption of the primitive belongs to Story 1.i1h's overflow-dialog work, not this one.
*   **And** the superseded per-segment "Day X of N" badge and `multiDayRoundingClass`'s edge-suppression logic are removed for the multi-day case now that one spanning element's position already conveys its range (DESIGN.md, revised 2026-09-14).

**Depends on:** Story 1.i1f.

**Note:** Split from Story 1.i1f via Gate 2 (`bmad-create-story`, 2026-09-17) — a genuinely new, non-trivial CSS-grid-column-spanning render path scoped to one component (`WeeklyCalendarView.tsx`'s `grid_weekly`), independently testable from the primitive itself. Implements the multi-day half of backlog.yaml IDEA-026 and EXPERIENCE.md's "Calendar View Cards: Attachment and Composition."
...
*   **Given** `Query.events`' current flat `ORDER BY (COALESCE next-upcoming-date) ASC LIMIT` week-level fetch can let one popular day consume the whole 1000-row budget and silently starve a later day in the same week,
*   **When** `CalendarView.tsx`'s week-level fetch runs,
*   **Then** the resolver applies a SQL window function partitioned per day (`ROW_NUMBER() OVER (PARTITION BY event_start_date ORDER BY event_start_time ASC NULLS LAST, id ASC) <= N`), guaranteeing every day its own fair slice, with multi-day segments and collapsed day-of-week runs excluded from this windowed set and fetched unconditionally alongside it.
*   **And** `N` is the EXPERIENCE.md-decided practical fallback (a flat 20 single-day/isolated occurrences per day), not a separately-chosen number.
*   **And** a single shared `calendar_overflow_dialog` component (DESIGN.md) replaces desktop's old static `max-h-56` popover entirely and replaces mobile's previous uncapped-always-render day-list rule — both surfaces cap their inline count (desktop: unchanged `max_events_per_day` (5), since single-day events still render via today's plain-text `CalendarCard` in `day_cell`, only the overflow surface's contents change; mobile: the same flat-20 bound) and open the shared dialog when exceeded, showing `Story 1.i1f`'s `EventCardCalendarGridItem` (no-image composition) with real infinite scroll.
*   **And** the dialog's own "load more for this day" pagination reuses `Query.events` unchanged (AD-2) — called with a DSL condition narrowed to that exact date (`scheduleDateRange overlaps {date, date}`) plus `offset`/`limit` continuing from the windowed fetch's per-day `N`.
*   **And** whenever the active DSL condition resolves to a single exact date, the resolver's outer `ORDER BY` applies an explicit secondary tie-break (`event_start_time ASC NULLS LAST, id ASC`) — both for the windowed week-fetch and for this reused pagination endpoint — so offset-based pagination across the two separate calls stays stable (a correctness precondition AD-23's own reviewer gate caught: without it, nothing guarantees a stable order between rows sharing the same computed next-upcoming-date once narrowed to one day).
*   **And** the precise per-render inline-cap measurement technique (e.g. `ResizeObserver`-based dynamic sizing) is explicitly deferred, not built here — EXPERIENCE.md itself sanctions the flat-bound fallback as an acceptable non-corner-cutting choice, not a shortcut around its own spec.
...
**Status: closed, no-go (2026-09-12) — will not be built as currently scoped.** Outreach was sent to storiesig.info and imginn.com mid-to-late August 2026; neither ever responded (4+ weeks of silence). A drafted storiesig.info follow-up nudge was never sent — the user chose to record a no-go decision rather than nudge further, mirroring Story 3.4b's own closure. mollygram.com was never contacted (deprioritized — wrong shape, single-item downloader not a per-account feed). See the full story file (`_bmad-output/implementation-artifacts/3-4c-explore-whitelisted-access-with-instagram-viewer-sites.md`) for the outreach email drafts, idea-protection guidance, and the closure record. Reopen only on a new, explicit decision (e.g. an unprompted vendor reply).
...
**Amendment (2026-09-03, added via `bmad-create-story`):** This story's own creation found a real contradiction in the AC text above and resolved it with the user via `AskUserQuestion` — the as-written ACs are superseded by the full story file, not this summary, but in brief: (1) classification runs **before** the first scrape (bio/username/displayName/`businessCategoryName` only — no post captions exist yet at subscribe time, so AC2's "mirror 3.4m's post-metadata trigger" as literally written is structurally incompatible with AC3's "excluded from the subscribe-time immediate scrape"); (2) `CURATOR_GUIDE` is **not** permanently excluded as AC3 originally stated — the user clarified curator accounts should eventually be scraped, just never with durable image storage or displayed/retained captions, a minimization pipeline split into new Story 3.4o since it doesn't exist yet and touches image storage/display code well beyond this story's own scope; until 3.4o ships, `CURATOR_GUIDE` is held back exactly like `PERSONAL`, never scraped without those protections; (3) a low-confidence/`AWAITING_APPROVAL` classification is treated as excluded (same as `PERSONAL`) while pending, never scraped optimistically; (4) legacy `SocialMediaAccountProfile` rows predating this story are explicitly **not** backfill-classified (user's explicit choice, accepted gap — `accountType`/`accountTypeStatus` stay `NULL` and are treated as an implicit allow, unchanged behavior); (5) the moderator review surface for `AWAITING_APPROVAL` accounts also doesn't fit the existing `/moderator/items` (Story 4.7) shape and is split into new Story 4.7c. See `_bmad-output/implementation-artifacts/3-4n-filter-scraped-accounts-by-type-organizer-venue-event-only.md` for the full, authoritative AC/task/Dev Notes detail — this epics.md entry is retained for FR traceability, not as the current spec.
...
*   **Given** a post is scraped from an account classified `accountType = 'CURATOR_GUIDE'` (Story 3.4n), **when** the post reaches Story 3.6e's image re-hosting step, **then** the durable re-host is skipped — `durableImageUrl` stays `null` for that post, exactly as it already does for a post that fails extraction — and the transient, short-lived source CDN URL is what's used until it expires (this story must confirm whether that's an acceptable end-state for `CURATOR_GUIDE`-sourced events, or whether a different, non-durable-but-longer-lived handling is needed — left for this story's own `bmad-create-story` pass to resolve, not decided here).
*   **And** a `CURATOR_GUIDE`-sourced post's caption is used as normal for Gemini event extraction (Story 3.6) but is never displayed anywhere in the app (event detail, manual post selection preview, or any other surface that currently renders raw post `content`) — this story must inventory every such display surface, not assume a single one.
*   **And** once a `CURATOR_GUIDE`-sourced post reaches a terminal extraction state (successfully extracted, or rejected/exhausted retries), its `posts.content` (caption) is cleared/nulled — this story must define the exact trigger point and confirm nothing downstream (moderator tooling, audit trails, re-extraction retry logic) still needs the raw caption after that point.
*   **And** once this story ships, Story 3.4n's scrape-gate for `CURATOR_GUIDE` accounts (currently "excluded, same as `PERSONAL`") is flipped to allow scraping — this story includes that gate flip, not a separate follow-up.
*   **And** this story's design must account for Story 3.6g/3.6h's separate, not-yet-built image-storage opt-in flag (moderator-granted consent for a specific account) — clarify whether `CURATOR_GUIDE` accounts are structurally incapable of that opt-in (never durably stored, full stop) or whether opt-in could someday override the `CURATOR_GUIDE` default; left to this story's own creation to resolve with the user, not assumed here.

**Note (2026-09-03, added via `bmad-create-story` during Story 3.4n's creation):** Story 3.4n's own creation found that "curator accounts can be scraped, just not with durable images or displayed captions" (the user's clarified policy, superseding 3.4n's original all-curator-accounts-excluded draft) requires a minimization mechanism that doesn't exist anywhere in the codebase today, and that spans image re-hosting (Story 3.6e) and multiple post/event display surfaces — well beyond 3.4n's own "which accounts do we scrape" scope. Confirmed via `AskUserQuestion`: `CURATOR_GUIDE` accounts are held back from scraping entirely (same as `PERSONAL`) until this story ships, so there is never a window where one is scraped without these protections. Positioned as a lettered suffix directly off Story 3.4n (`story-split-gate.md`'s "single-story split" numbering rule — 3.4n's own creation is what surfaced this gap).
...
**Cross-reference (2026-09-03, added via `bmad-create-story` during Story 3.4o's creation):** Story 3.4o (Gate 3 finding) already ships a narrower, `CURATOR_GUIDE`-specific version of this story's own gate — `apps/backend/src/lib/ai-processor/process-ai-job.ts`'s `rehostPostImageSeam` call skipped when `accountType === 'CURATOR_GUIDE' && !isImageStorageOptedIn`. When this story is created, read Story 3.4o's actual shipped condition (`_bmad-output/implementation-artifacts/3-4o-minimize-stored-displayed-data-for-curator-guide-sourced-posts.md`, Task 3) and generalize/replace it with this story's `isImageStorageOptedIn`-for-all-accounts condition, rather than layering a second, redundant condition beside it at the same call site.
...
**Amendment (2026-09-04, added via `bmad-create-story` while drafting this story):** Gate 1 (run fresh via `runSubagent`, Winston persona — Epic 3's swept `epic-3-readiness.md` predates this story and Stories 3.7c/3.7d were never covered by it) found this story cannot be implemented frontend-only: AC3/AC4's opt-in-aware failure branching needs a typed success/error signal from Instagram's oEmbed mechanism, which the client-side `embed.js` widget does not provide, and any direct call to Meta's oEmbed API from the browser is additionally blocked outright by Instagram's `Cross-Origin-Resource-Policy: same-origin` response header regardless of credentials. Live research during this story's creation found Meta reversed its 2020 access-token requirement on 2026-06-15 — `GET https://graph.facebook.com/v25.0/instagram_oembed?url=<post_url>` now works tokenless, no App Review, for public posts, returning JSON (`html` on success, a typed error for deleted/private posts), documented at ~1,000 req/hour — so no Meta App credential/IaC is needed, only a backend-proxied call. Split into new prerequisite **Story 3.7e** (adapter + `Event.instagramEmbed` resolver field + rate-limit-safe caching) rather than the frontend calling Instagram directly or absorbing backend logic into this story. User confirmed the single-combined-story split (rather than separately splitting out credential provisioning, moot given the tokenless endpoint) via direct confirmation during this story's creation. Gate 2 (run fresh via `runSubagent`, Freya persona) additionally found the embed itself needs a new dedicated `InstagramEmbed.tsx` component (its own loading/ready/unavailable/fallback state machine) rather than another branch bolted onto `EventImage.tsx`'s existing image/video state machine, plus confirmed no existing `DESIGN.md`/`EXPERIENCE.md` content specifies this embed's visual treatment — both folded directly into this story's own AC/task list below (not a separate story, since both are single-consumer, story-3.7d-only concerns). Gate 3 (run fresh, Winston persona) found no need to generalize this into a shared `EmbedAdapter` interface yet (only one consumer exists; Story 3.7c stays hotlink-only with no embed concept), deferring generalization until a second embed-needing platform story actually appears.

**Depends on:** Story 1.6a (EventDetailView), Story 3.6h, Story 3.7e (new prerequisite, added 2026-09-04).
...
*   **Given** a `Post.originalPostUrl` (or `sourcePostUrl` when it is an Instagram URL) for an event's source post, **when** a new `InstagramOEmbedAdapter` (`apps/backend/src/lib/adapters/instagram-oembed-adapter.ts`, following this project's established external-service adapter pattern) is called with that URL, **then** it calls `GET https://graph.facebook.com/v25.0/instagram_oembed?url=<url>` (no access token required — Meta's 2026-06-15 tokenless reversal) and returns a typed result: `{ status: 'AVAILABLE', html: string }` on a successful JSON response, or `{ status: 'UNAVAILABLE' }` for any non-2xx/error response (deleted post, private account, invalid URL, or any other adapter failure) — mirroring the project's existing best-effort external-call patterns (e.g. `rehostPostImageSeam`'s failure handling, Story 3.6e).
*   **And** the adapter's calls are cached (e.g. keyed by post URL, a bounded TTL) so that repeated requests for the same event within the cache window do not re-call Instagram's endpoint, keeping usage well under Meta's documented ~1,000 requests/hour tokenless budget as event-detail traffic grows.
*   **And** a new `Event.instagramEmbed` GraphQL field (not exposed as a raw pass-through of the adapter — computed by a resolver) returns a typed union/shape the frontend can render directly: on `AVAILABLE`, the embed `html`; on `UNAVAILABLE` for a NON-opted-in account (`isImageStorageOptedIn = false`, joined the same way Story 3.6h's `Event.imageUrl` resolver already joins it), a status indicating no fallback is available (frontend renders "content no longer available", per Story 3.7d AC3); on `UNAVAILABLE` for an OPTED-IN account, a status carrying `Post.durableImageUrl` as the fallback (per Story 3.7d AC4) — reusing the exact opt-in join/flag Story 3.6h's `Event.imageUrl` resolver already established, not a second parallel opt-in check.
*   **And** this field is resolved only when requested (GraphQL field-level resolution — no eager fetch on every `Event` query), since it triggers a real external network call and should not slow down list/grid queries that never render an embed.
...
**Cancelled (2026-08-29, via `bmad-correct-course`):** User decision reverses this story's own premise — reporting now requires viewing an event's detail view first, so a card-level report trigger is deliberately not wanted. PRD 3.9.2's "list-view or detailed view" clause (the requirement this story was split off Story 4.3 to satisfy) has been narrowed to detail-view only. No code was written for this story (`ready-for-dev`, unstarted) — nothing to roll back. Story 4.3's existing detail-view report flow, Story 4.3a's backend, and Story 4.3c's server-side list-visibility exclusion are all unaffected. See `sprint-change-proposal-2026-08-29.md`.
...
*   **Given** `/moderator/items` (Story 4.7, extended by 4.7b), **when** a moderator opens it, **then** a new list type/tab surfaces every `accountTypeClassificationReviews` row with no `reviewedAt` set — showing the account's username/displayName/bio snippet, the proposed `accountType` and confidence score (if the classification attempt produced one), and the `failureReason` (if it exists because of a hard failure rather than low confidence).
*   **And** the moderator can resolve a pending row by choosing one of the three real `accountType` values (`ORGANIZER_VENUE_EVENT`/`PERSONAL`/`CURATOR_GUIDE`) via a new mutation that sets `socialMediaAccountProfiles.accountType`/`accountTypeStatus = 'CONFIRMED'` directly and stamps the `accountTypeClassificationReviews` row's `resolvedAccountType`/`reviewedByModeratorId`/`reviewedAt`.
*   **And** resolving to `ORGANIZER_VENUE_EVENT` makes the account immediately eligible for the next scrape trigger (batch, on-demand, etc. — Story 3.4n's gate) — this story does not itself trigger an immediate scrape, it only clears the gate for the next natural trigger.
*   **And** this is a genuinely new list/resolve shape, not a reuse of `pendingDefaultLocationChanges`'s query/mutation (different data: a proposed classification/confidence, not a location diff) — Gate 2 (UX) confirmed during Story 3.4n's creation that `/moderator/items` today hard-codes exactly two list types with no generic plug-in mechanism, so this adds a third, following the same page-level pattern rather than generalizing the page itself (out of scope here).

**Note (2026-09-03, added via `bmad-create-story` during Story 3.4n's creation):** Gate 2 (Freya/UX persona, run as part of Story 3.4n's own creation) found that `/moderator/items`'s existing `reportedEvents`/`pendingDefaultLocationChanges` list types are each hard-coded with their own query, mutation, and type-specific fields/actions — there's no existing slot an account-type-classification review can drop into without new work, matching how FR94-96 already had to extend Story 4.7 itself for a second Default-Location sub-case rather than being absorbed into whichever story triggered that need. Positioned as a lettered suffix off **Story 4.7** (the story that owns `/moderator/items`), not off 3.4n, since it extends 4.7's own page/query/mutation pattern, not 3.4n's scrape-gating logic.

**Depends on:** Story 3.4n, Story 4.7, Story 4.7b.
...
*   **And** it fails if `geocodeAddress`'s address-search path stops choosing among its retained candidates by `confidence`/`matchType` rather than position (Story 0.i7b's re-ranking).
*   **And** it fails if the event-detail map-link builder (`mapper.ts`) renders a coordinate link without first checking `confidence >= 0.5` and `matchType === 'full_match'` (Story 0.i7c's gate).
*   **And** it fails if any of the five GraphQL operations Story 0.i7d instrumented (`setAccountDefaultLocation`, `editAccountDefaultLocation`, `createUserLocation`, `updateUserLocation`, `previewLocation`) stops selecting `confidence`/`matchType` in its `.graphql` operation document — enforced by that story's three AST guard tests remaining present and passing in the normal test suite.
