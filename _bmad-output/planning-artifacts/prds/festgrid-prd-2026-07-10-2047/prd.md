...
*   **Multi-Image Extraction Stays Within One Call (added 2026-09-03; FR104):** Section 3.7's multi-image (carousel) extraction sends a post's cover image plus additional slide images as multiple parts of a **single** request, never as separate sequential requests per image — confirmed during design as the quota-efficient choice, since Gemini's free-tier quota is bounded by RPM/RPD (request count) as well as token throughput, and batching avoids multiplying request count for a single post's extraction. A configurable per-post image-count cap (default 5) bounds worst-case token volume and latency regardless of how many images a source post actually has.
...
interface Post {
   ...
   * in slide order (added 2026-09-03, FR104). Extraction-time input only: used by AI extraction
