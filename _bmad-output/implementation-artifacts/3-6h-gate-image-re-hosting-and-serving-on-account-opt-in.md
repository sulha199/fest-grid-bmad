...
- [x] Scope confirmation — one condition generalized in `process-ai-job.ts` (Task 1); one required parameter added to an existing pure `packages/domain` function plus its logic/tests (Task 2); 5 existing resolver query sites gain one join + one field each, plus the `Event.imageUrl` field resolver updated to pass the new param (Task 3). No new files, no new DB migration, no new GraphQL API surface, no `apps/web` change.
- [x] Architecture and boundary confirmation — the domain-layer gate stays pure in `packages/domain` (no DB/ORM coupling introduced there); the DB join and row-shape wiring stay in `apps/backend/src/schema/resolvers.ts`, matching this file's existing pattern for every other joined field (`durableImageUrl`, `imageUrlExpiresAt`, `sourceSocialMediaAccountProfile`).
- [x] Testing plan confirmation — `packages/domain`: 100% branch coverage of the updated `resolveServedImageUrl` (Task 2); `apps/backend`: updated `process-ai-job.test.ts` fixture + new not-opted-in regression case (Task 5), new `resolvers.test.ts` end-to-end resolver cases (Task 6), as specified in the Verification Plan above.
...
- [x] E2E tests: not required — this is a backend-only data-correctness fix with no new user-facing flow to exercise; the existing frontend placeholder behavior for a null image is already covered by whatever E2E/integration coverage exists for that pre-existing state (not newly introduced by this story).

## Deliverables Checklist

- [x] `process-ai-job.ts`'s rehost-skip gate generalized from CURATOR_GUIDE-only to all accounts (AC1, AC2).
- [x] `resolveServedImageUrl` gated on a new required `isImageStorageOptedIn` param, with the still-valid-original tier left ungated (AC3, AC4, AC5).
- [x] All 5 `resolvers.ts` Event-row query sites join `socialMediaAccountProfiles` and pass `isImageStorageOptedIn` into the `Event.imageUrl` field resolver.
- [x] No new GraphQL API surface, no new DB migration, no `apps/web` change.
- [x] `packages/domain` unit tests at 100% coverage for the updated function; `apps/backend` integration tests for both the pipeline gate and the resolver gate.
...
- [x] AC1-6 satisfied.
- [x] All required tests passing (`packages/domain` unit tests at 100% coverage for the updated function; `apps/backend` integration tests for both the generalized pipeline gate and the new resolver gate).
- [x] Lint and type checks passing for `packages/domain` and `apps/backend`.
- [x] No `apps/web`/`packages/ui`/database-migration/GraphQL-schema changes introduced (this story is scoped to be backend-logic-only — any diff outside `packages/domain`/`apps/backend` should be treated as scope creep and questioned).
- [x] The Pre-Coding Approval Gate's flagged Story 3.6g dependency/sequencing risk is resolved (3.6g committed, or both stories' uncommitted work is being tracked/committed together) before this story is marked done.
...
- Fixed a pre-existing type mismatch in `apps/backend/src/lib/posts/enqueue-post-for-processing.ts` where nullable `post.content` was assigned to a strict `string` field; resolved by using `post.content ?? ''`.
- Found that `@festgrid/domain` workspace package exports `./dist/events/index.js` which is compiled. Successfully resolved TypeScript build caching / test execution failures by rebuilding `@festgrid/domain` using `pnpm --filter @festgrid/domain build` before running backend schema tests.

### Completion Notes List

- Generalized the pipeline re-hosting skip gate in `apps/backend/src/lib/ai-processor/process-ai-job.ts` from CURATOR_GUIDE-only to all accounts.
- Added a required `isImageStorageOptedIn: boolean` parameter to the pure `resolveServedImageUrl` domain function in `packages/domain/src/events/resolveServedImageUrl.ts`, and updated its logic according to AD-12 Rule 3.
- Integrated the new parameter into 5 Event query select sites in `apps/backend/src/schema/resolvers.ts` via a `.leftJoin` with `socialMediaAccountProfiles` on `posts.accountId`.
- Updated `Event.imageUrl` field resolver to pass `isImageStorageOptedIn` from the parent row.
- Updated `@festgrid/domain` unit tests with `isImageStorageOptedIn` and wrote 4 new cases covering all branches at 100% test coverage.
- Wrote new integration tests in `process-ai-job.test.ts` for plain non-CURATOR_GUIDE account skips.
- Wrote comprehensive end-to-end integration tests in `resolvers.test.ts` for AC3, AC4, and AC5 resolver behaviors.
- Confirmed type-checking (`tsc`), linting, and Node integration tests are 100% passing and green across all affected modules.
...
- `_bmad-output/implementation-artifacts/3-6h-gate-image-re-hosting-and-serving-on-account-opt-in.md`
