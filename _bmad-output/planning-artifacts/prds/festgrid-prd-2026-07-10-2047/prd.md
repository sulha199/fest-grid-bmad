...
*   **Multi-Image Extraction Stays Within One Call (added 2026-09-03; FR112):** Section 3.7's multi-image (carousel) extraction sends a post's cover image plus additional slide images as multiple parts of a **single** request, never as separate sequential requests per image — confirmed during design as the quota-efficient choice, since Gemini's free-tier quota is bounded by RPM/RPD (request count) as well as token throughput, and batching avoids multiplying request count for a single post's extraction. A configurable per-post image-count cap (default 5) bounds worst-case token volume and latency regardless of how many images a source post actually has.
...
enum EventCategory {
  ...
  TECHNOLOGY,             // Tech product promos, hackathons, tech meetups/conferences (New)
  TRAVEL_AND_TOURISM,     // Travel gear promos, travel fairs, tourism expos (New)
  EDUCATION,              // Academic olympiads/competitions, school contests (New)
  CAREER,                 // Job fairs, career workshops (New)
  AUTOMOTIVE,             // Car and motorcycle fairs, exhibitions (New)
  OTHER
}
...
interface Schedule {
   ...
   */
   id: string;
  /**
   * A unique, URL-friendly slug, generated as a random hex string (`randomBytes(6).toString('hex')`
   * -- previously and incorrectly documented here as "Nano ID"; it was never Nano ID). Unlike
   * `EventInfo.slug` (see above), `Schedule.slug` does not use the platform-prefixed scheme --
   * a schedule has no direct social-media-post identity of its own. Supports manual custom
   * modification post-MVP (Section 8.2).
   */
   slug: string;
  /**
...
interface Post {
  ...
  imageUrl?: string;
  /**
   * URLs of a carousel/Sidecar post's additional slide images beyond the cover (`imageUrl`),
   * in slide order (added 2026-09-03, FR112). Extraction-time input only: used by AI extraction
   * (Section 3.7) to find schedule information that lives on a later slide rather than the cover
   * image or caption. Never displayed in any UI, never durably re-hosted (Architecture Spine
   * AD-12/AD-13 — distinct from `imageUrl`'s re-hosting treatment). Absent/empty for non-carousel
   * posts.
   */
...
Provide premium users with a user interface to view and modify their autogenerated slug on EventInfo and Schedule resources. A custom slug replaces whichever autogenerated form the resource had (the platform-prefixed scheme for a post-sourced `EventInfo`, or the random-hex fallback for `Schedule`/a sourceless `EventInfo` -- see Section 4.1/4.4 and Architecture Spine AD-16); no fallback logic is needed to keep a resource's slug in the platform-prefixed format once a custom slug is set.
