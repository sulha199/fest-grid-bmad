...
12. **AC12 — Adopt `PageContainer(fullWidth=false)`/`PageHeader` (added 2026-08-24 via `bmad-correct-course`):** And `api-keys-content.tsx`'s root `<div className="p-4 sm:p-8 space-y-8 max-w-3xl mx-auto">` (all 3 occurrences in this file — loading skeleton, error state, and success return — for visual consistency across all three render branches) is replaced with `<PageContainer fullWidth={false}>` (`@festgrid/ui`, Story 0.30), and its `<h1 className="text-3xl font-bold">{t("title")}</h1>` row is replaced with `<PageHeader title={t("title")} />` (Story 0.32), **including its existing header "Add API Key" button as the `action` prop** (`action={{ label: t("addButtonLabel"), icon: <Plus className="h-4 w-4" />, onClick: () => setIsDialogOpen(true) }}`) — this page's only trigger for the add-key dialog is (and has always been) that header button; there is no separate in-table add-row anywhere in this file. **Depends on Story 0.30 (AC7) and Story 0.32.**

    *(Correction 2026-08-24, post-Wave-4 verification: this AC's text originally read "this page has no header action button (adding a key is via the in-table add-row), so no `action` prop" — that in-table add-row never existed; a dev agent that implemented the text literally silently deleted the only way to open `ApiKeyFormDialog`, caught by this file's own existing `api-keys-content.test.tsx` ("allows clicking Add API Key to open form...") once actually run. Fixed directly rather than reopening a new AC.)*
...
**2026-08-24 (Wave 4):** AC12 delivered via Cline, but its dev report ("PASSED with zero findings," no mention of running tests) did not hold up on independent inspection — the header's "Add API Key" button had been silently dropped, per AC12's own inaccurate premise (see the correction note on AC12 above). Fixed directly during verification (action prop restored) and confirmed against this file's own existing test suite, which the fix now passes 4/4. Merged to `master`.
