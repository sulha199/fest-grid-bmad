...
**AC13 — Carousel-chrome Next/Previous navigation with peek previews (amended 2026-08-16 via `bmad-correct-course`):**
...
- [x] 2. Add `slug: String!` to the `Event` type in `apps/backend/src/schema/events.graphql` (AC4) — the DB column (`events.slug`, unique/notNull since Story 1.1) and `packages/shared-types`' `EventInfo.slug` already exist; only the GraphQL exposure is missing. No resolver code needed — `buildOptimizedDrizzleSelect` auto-selects it once it's a requested GraphQL field matching a real column.
- [x] 3. Add `eventBySlug(slug: String!): Event` to the `Query` type in `events.graphql`, and implement `Query.eventBySlug` in `apps/backend/src/schema/resolvers.ts`, mirroring the existing `Query.event` resolver's pattern (`buildOptimizedDrizzleSelect(events, info)` + `eq(events.slug, slug)`) (AC4).
- [x] 4. Regenerate backend types (`pnpm --filter backend codegen`) and add/extend integration tests in `apps/backend/src/schema/resolvers.test.ts` for `eventBySlug`: found-by-slug, not-found returns `null`, field-selection only returns requested columns (AC10).
- [x] 5. Add an `eventBySlug` GraphQL operation document under `apps/web` and regenerate the web client (`pnpm --filter web codegen`) to produce a typed `useEventBySlugQuery` hook (AC4).
- [x] 6. Update `apps/web/src/app/[locale]/layout.tsx` to accept and render a `modal` parallel-route slot alongside `children` (inside `<AppShell>`), and add the required `apps/web/src/app/[locale]/@modal/default.tsx` (renders `null`) (AC1, AC2).
- [x] 7. Create the full-page route as a Server Component `apps/web/src/app/[locale]/events/[slug]/page.tsx` holding `export async function generateMetadata({ params })` — resolve `slug`/`locale`, fetch the event server-side (or accept a `null`/not-found title fallback if only client-fetching is wired), call `getTranslations({ locale, namespace: 'Metadata' })` (or a new event-scoped namespace) and `buildPageMetadata(...)` (Story 1.9's shared helper) to build a title/description from the event's `eventName`/description, with baseline `og:title`/`og:description` (AC12) — then render the extracted client logic from a colocated `event-detail-content.tsx` (Task 7a).
- [x] 7a. Create `apps/web/src/app/[locale]/events/[slug]/event-detail-content.tsx` (`"use client"`): fetch via `useEventBySlugQuery`, map the GraphQL `Event` shape onto `EventDetailView`'s prop contract (a shared mapper, not duplicated per route), render `EventDetailView` directly (no modal chrome), and render a localized "not found" state when the query resolves `null` (AC2, AC3, AC8) — mirrors Story 1.9's `page.tsx`/`home-content.tsx` split, needed here because `generateMetadata` cannot be exported from a `"use client"` file.
- [x] 8. Create the intercepted modal route as a Server Component `apps/web/src/app/[locale]/@modal/(.)events/[slug]/page.tsx` holding its own `generateMetadata` (same pattern as Task 7, AC12) — then render the extracted client logic from a colocated `event-detail-modal-content.tsx` (Task 8a).
- [x] 8a. Create `apps/web/src/app/[locale]/@modal/(.)events/[slug]/event-detail-modal-content.tsx` (`"use client"`): same fetch + mapping as Task 7a (reuse the shared mapper), wrap `EventDetailView` in the existing Shadcn `Dialog` (`apps/web/src/components/ui/dialog.tsx`), with `onOpenChange(false)` calling `router.back()` to dismiss and restore the underlying URL (AC1, AC3).
...
- [x] **Task 15 (AC13) — Wrap Next/Previous in shadcn `Carousel` chrome with peek previews (`apps/web`):**
  - `pnpm --filter web exec shadcn add carousel` installed `embla-carousel-react` and `apps/web/src/components/ui/carousel.tsx` — `apps/web`'s own already-existing `components.json`/`cn()`/`@/*` alias, no Story 0.28 dependency, exactly like `apps/web/src/components/ui/dialog.tsx`.
  - `EventDetailWrapper.tsx` wraps a 3-slide `CarouselContent` window: a previous-peek `CarouselItem` (rendered only when `!nav.previous.disabled`), the current-slide `CarouselItem` (unchanged `EventDetailView` loading/error/populated branches), and a next-peek `CarouselItem` (rendered only when `!nav.next.disabled`). `CarouselPrevious`/`CarouselNext`'s click handlers stay wired to the **existing** `handlePrevious`/`handleNext` (Story 1.6b's `nav.requestNext()`/`router.replace`), never Embla's own `scrollPrev`/`scrollNext` — Embla's slide-index state is consulted only via `emblaApi.selectedScrollSnap()` inside a `select`-event listener (obtained via `Carousel`'s `setApi` prop) that itself calls the same `handleNext`/`handlePrevious`, letting a real swipe gesture commit navigation through the identical code path as a button click. A `useEffect` re-centers Embla (`scrollTo(currentSlideIndex, true)`, no animation) whenever the current event or the window's shape changes, so the visual position always matches the real "current" slide after a commit.
  - `(amended 2026-08-16 via bmad-correct-course, sprint-change-proposal-2026-08-16-detail-carousel-ux-fixes.md)`: two follow-up fixes to the initial peek-preview implementation:
    - `EventPreviewCard` (`apps/web/src/features/events/event-preview-card.tsx`) restructured to mirror `EventDetailView`'s own `loading`-skeleton proportions (title bar, tag-pill bars, one large content block) instead of a generic image + 3-bar shape — the active and peek slides no longer visibly jump in size/structure while swiping. The real image (already available from the list-navigation hook's target item, zero extra fetch) still replaces only the top skeleton block when present.
    - `EventDetailWrapper.tsx` prefetches `nav.previous.target`/`nav.next.target`'s routes via `router.prefetch()` as soon as they're known — this warms the full-page route's dynamic payload (no sibling `loading.tsx` there) but, as later discovered, does **not** warm a route that has its own `loading.tsx` (Next.js only prefetches down to and including that boundary, never what's behind it).
...
- [x] **Source-post-link requirement reinstated (2026-08-01 correction) acknowledged:** the previous draft's ungrounded "proxy-platform post URL"/missing-storage claims were corrected via `bmad-correct-course`, not simply dropped — AC11 now requires `originalPostUrl`/`postUrl` attribution links, grounded in PRD §3.3.3/§3.7/§4.7. Confirm Story 1.2a's AC7/Task 13 (`posts.original_post_url` migration) lands before/alongside this story, or explicitly accept temporarily shipping with `originalPostUrl` always `null` (the column would simply not exist yet).
- [x] Architecture and API/data boundaries confirmed (GraphQL-only, non-DSL single-item query; AD-1/AD-2/AD-4/AD-5/AD-6).
...
- [x] All Acceptance Criteria (AC1–AC14) are met (AC1-12 already were, per prior implementation; AC14 already shipped; AC13 shipped via Task 15, amended 2026-08-16 for peek-shape/RouteLoader fixes).
