...
6. **Task 6 - Frontend Bounded Polling:** Implemented conditional polling logic:
   - Added `scrapePollingStartTime` state to track trigger moment
   - Modified `useGetMySubscriptionsQuery` to use `refetchInterval: 3000` while `isScrapeInProgress` is true
   - Implemented 60-second timeout check (`pollTimeoutMs`) to stop polling and show timeout message
   - Added `useEffect` to refetch posts when `isScrapeInProgress` transitions false

7. **Task 7 - Frontend UI Control:** Built `renderScrapeControl()` helper component with:
   - Button in empty state showing `scrapePostsEmptyStateCta` message
   - Persistent tab-bar control showing when posts exist
   - Disabled state with spinner while `isScrapeInProgress` or `isScraping`
