...
- Story ID: 1.3g
- Status: ready-for-dev (AC15 amendment; AC1-AC14 already delivered — see Dev Notes → Amendment (2026-08-26))
...
- **Downstream follow-up for Stories 1.3f/2.6 (both already implemented — `review`/`done` respectively):** the new `labels.multiDaySegmentLabel` resolver is optional with a plain-string fallback, so neither wrapper's `CalendarView.tsx`/`my-calendar-content.tsx` call site strictly needs a code change for this AC to function — the mobile list will render correctly with the English fallback text even if untouched. However, for full i18n parity (matching this story's own AC9 intent and the precedent set by AC13's `getWeekRange` follow-up), both wrapper files should eventually be updated to pass a real `next-intl`-wired resolver (`multiDaySegmentLabel: (dayNumber, totalDays) => t('calendarMultiDaySegmentLabel', { dayNumber, totalDays })` or equivalent ICU-plural-aware message) plus the corresponding `apps/web/locales/{en,id}.json` key — flagged here for whenever 1.3f/2.6 are next reopened, not absorbed into this story (mirrors exactly how AC13's `getWeekRange` prop was flagged for the same two files in the "Current Implementation State (AC13...)" Dev Notes section above).
...
- **Stories 1.3f/2.6's own `multiDaySegmentLabel` i18n wiring** (AC15) — optional, falls back to a plain default string; flagged for their own follow-up when next reopened, not built here (see Dev Notes → Amendment (2026-08-26)).
