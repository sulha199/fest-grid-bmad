...
- [ ] 7. Create the full-page route `apps/web/src/app/[locale]/events/[slug]/page.tsx`: fetch via `useEventBySlugQuery` (or a server-side equivalent), map the GraphQL `Event` shape onto `EventDetailView`'s prop contract (a shared mapper, not duplicated per route), render `EventDetailView` directly (no modal chrome), and render a localized "not found" state when the query resolves `null` (AC2, AC3, AC8).
- [ ] 8. Create the intercepted modal route `apps/web/src/app/[locale]/@modal/(.)events/[slug]/page.tsx`: same fetch + mapping as Task 7 (reuse the shared mapper), wrap `EventDetailView` in the existing Shadcn `Dialog` (`apps/web/src/components/ui/dialog.tsx`), with `onOpenChange(false)` calling `router.back()` to dismiss and restore the underlying URL (AC1, AC3).
...
- [ ] 11. Add message keys to `apps/web/locales/en.json` and `apps/web/locales/id.json` for this story's own page-level copy (e.g. `EventDetailsPage.next`, `.previous`, `.closeModal`, `.notFoundTitle`, `.notFoundBody`) and populate `EventDetailView`'s `labels` prop with translated strings at each route's call site (AC8).
...
- `apps/web/src/app/[locale]/@modal/(.)events/[slug]/page.tsx` — NEW (modal route).
- `apps/web/src/app/[locale]/events/[slug]/page.tsx` — NEW (full-page route).
